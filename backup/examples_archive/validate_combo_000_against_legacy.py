"""
Validate Combo 000 Against Legacy Code
Compare combo_000.parquet data with exact same computation using predictors_tools legacy functions
"""
...
def load_original_legacy_data():
    ...
    try:
        ...
        print("🔄 Converting to 15-minute OHLCV format...")
        ...
        # The index name after reset_index becomes a column - rename it to 'datetime'
        trades_df.columns = ['datetime', 'price']  # Ensure proper column names
        ...
        print(trades_df.head(2).to_string(index=False))
...
def load_combo_000_data():
    """Load combo_000.parquet and its parameters"""
    print("\n📊 Loading combo_000 data...")
    ...
    try:
        ...
        combo_0 = metadata[metadata['combo_id'] == 0].iloc[0]
        
        print(f"✅ Loaded combo_000: {combo_data.shape}")
        print(f"   MACD params: short={combo_0.macd_short}, long={combo_0.macd_long}, signal={combo_0.macd_signal}")
        print(f"   ATR lookback: {combo_0.atr_lookback}")
        
        return combo_data, combo_0
        
    except Exception as e:
        print(f"❌ Failed to load combo data: {e}")
        return None, None

def compute_legacy_indicators(trades_df, historical_candles, combo_params):
    ...
    try:
        ...
        print(f"   Columns: {list(legacy_macd.columns)}")
        ...
        print(f"   Columns: {list(legacy_atr.columns)}")
        
        # Show sample of legacy results (use correct column names)
        print(f"\n📋 Sample legacy MACD data:")
        print(legacy_macd[['datetime', 'macd', 'signal', 'hist']].head(3).to_string(index=False))
        
        print(f"\n📋 Sample legacy ATR data:")
        print(legacy_atr[['datetime', 'atr']].head(3).to_string(index=False))
        
        return legacy_macd, legacy_atr
...
def compare_indicators(combo_data, legacy_macd, legacy_atr, tolerance=1e-10):
    ...
    for combo_col, legacy_col in macd_comparisons.items():
        if combo_col not in combo_data.columns:
            ...
            continue
        if legacy_col not in legacy_macd.columns:
            print(f"⚠️  Column {legacy_col} not found in legacy MACD data")
            continue
        
        # Get non-null values
        combo_values = combo_data[combo_col].dropna()
        legacy_values = legacy_macd[legacy_col].dropna()
    ...
    # Compare ATR
    print("\n📊 ATR Comparison:")
    if 'atr' in combo_data.columns and 'atr' in legacy_atr.columns:
        combo_atr = combo_data['atr'].dropna()
        legacy_atr_values = legacy_atr['atr'].dropna()
...
def main():
    ...
    print("🔬 COMBO 000 vs LEGACY PREDICTORS_TOOLS VALIDATION")
    print("=" * 70)
    print("This validates combo_000.parquet against exact same computation")
    print("using the correct legacy predictors_tools functions as specified")
    print("in the fix handoffs.")
    print()
    
    if not LEGACY_FUNCTIONS_AVAILABLE:
        print("❌ Legacy functions not available - cannot perform validation")
        return False
    
    # Load original legacy data
    trades_df, historical_candles = load_original_legacy_data()
    if trades_df is None or historical_candles is None:
        return False
    
    # Load combo_000 data and parameters
    combo_data, combo_params = load_combo_000_data()
    if combo_data is None or combo_params is None:
        return False
    
    # Compute indicators using legacy functions with exact same parameters
    legacy_macd, legacy_atr = compute_legacy_indicators(trades_df, historical_candles, combo_params)
    if legacy_macd is None or legacy_atr is None:
        return False
    
    # Compare results
    comparison_results = compare_indicators(combo_data, legacy_macd, legacy_atr)
    ...
    if successful_matches == total_comparisons and total_comparisons > 0:
        print(f"\n🎉 SUCCESS! Combo 000 matches legacy predictors_tools computation")
...
if __name__ == "__main__":
    ...
    if success:
        print(f"\n🎊 VALIDATION COMPLETE: Combo 000 verified against legacy code!")
    else:
        print(f"\n🚨 VALIDATION FAILED: Combo 000 does NOT match legacy computation!")
