...
for grid in grids:
    ...
    if os.path.exists(inst_cap_filepath):
        ...
        start_date = last_date - timedelta(days=1) if last_date <= current_day else current_day - timedelta(days=7)
...
# Process the stored data and update the database
data_dir = inst_cap_path
db = Database()
...
def get_fcst_date_range(schema_name, table_name):
    ...
    return pd.date_range(start_date, datetime.today().replace(hour=0, minute=0, second=0, microsecond=0), freq='12H')
