...
end_date = datetime.today().replace(hour=0, minute=0,
                                    second=0, microsecond=0) + timedelta(days=365)
...
for grid in grids:
    ...
    if os.path.exists(inst_cap_filepath):
        updated_data = pd.concat([existing_data, new_data_df]).sort_index()
        last_row = updated_data.iloc[[-1]]
        updated_data = updated_data.drop_duplicates()
        updated_data = pd.concat([updated_data, last_row])
