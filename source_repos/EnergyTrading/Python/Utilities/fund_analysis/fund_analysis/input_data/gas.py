...
class Gas(input_data):
    ...
    def update_live_prices_from_file(self, market_df=None):
        ...
        if today_date.hour < 8:
            market_df.loc[self.pivot_date] = market_df.iloc[-1]
            return market_df
        else:
            today_date = today_date.replace(hour=0, minute=0, second=0, microsecond=0)
            file_path = '//10.123.0.5/algodata/LiveScreen_v01.xlsm'  # Replace with the path to your file
    ...
    def get_curve(self, date_shift=-1):
        self.get_fut_matrix()
        for market, market_df in self.fut_matrix.items():
            ...
            if not market in self.data_curve:
                ...
                curve = market_df.loc[:(self.pivot_date-dt.timedelta(days=date_shift)), :].iloc[-1].copy()
                curve_last_date = curve.name
                ...
                dates_list = [start_date(curve_last_date, product)
                            if product.lower() not in ['wk', 'wknd'] else
                            start_date(curve_last_date, 'w') if product.lower() in ['wk'] else None
                ...
                da_value = self.da_data[market].loc[:(self.pivot_date-dt.timedelta(days=date_shift)), :].iloc[-1].copy()
                # Step 3: Calculate the number of days between the pivot_date and the first date of the curve
                first_curve_date = curve.index[0]
                days_to_interpolate = max(0,(first_curve_date - self.pivot_date).days + 15) 
                if days_to_interpolate > 0: # Add 15 extra days for interpolation
    ...
    def get_da_data(self):
        for market in self.gas_markets:
            ...
            if today_datetime.hour < 8:
                da_temp.loc[last_date + pd.Timedelta(days=1)] = da_temp.iloc[-1]
