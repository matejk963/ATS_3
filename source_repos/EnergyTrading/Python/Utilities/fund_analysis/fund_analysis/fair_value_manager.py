...
from fund_analysis.utils import spreads_creation
...
class FairValueManager:
    ...
    def get_fcst_curves(self,model_params_dict:dict,
        ...
        for fcst_date in self.fcst_range:
            ...
            # Save scenarios dict
            folder_path = r'C:\Users\krajcovic\Documents\Trading\fund_analysis\model\base_data'
            file_name = f"scen_dic_{fcst_date.strftime('%Y%m%d')}.pkl"
            ...
            # Save a model
            folder_path = r'C:\Users\krajcovic\Documents\Trading\fund_analysis\model\saved_models'
            for market, market_model in model_inst.fitted_model.items():
                file_path = os.path.join(folder_path,
                                         f"{market}_xgbregressor_{fcst_date.strftime('%Y%m%d')}.json")
                market_model.save_model(file_path)
            curve_pred_nominal = {}
            print(f"Modeling ends at {dt.datetime.now()}")
            spreads_creation.create_spreads({fcst_date:curve_pred},
                                            list(np.unique(self.params_dict['product_list'])),
                                            'xgboost_mean',
                                            spread_type_list=['outright_prices'])
            
            # for grid, pred_df in curve_pred.items():
            #     table_name = fcst_date.strftime('%Y-%m-%d')
            #     stage_name = f"stage_{table_name}"
            #     schema_name = f"zFCST_raw_{grid}"
            #     pred_df.reset_index().to_sql(name=stage_name,
            #               schema=schema_name,
            #               con=db.connection_string,
            #               if_exists='replace', index=False)
            #     db.merge_from_staging_to_prod(schema_name, table_name)
            ...
            spreads_creation.create_spreads({fcst_date:curve_pred_nominal},
                                            list(np.unique(self.params_dict['product_list'])),
                                            'xgboost_mean_nominal',
                                            spread_type_list=['outright_prices'])
            # spreads_creation.create_spreads({fcst_date:curve_pred},
            #                                 list(np.unique(self.params_dict['product_list'])),
            #                                 'xgboost_mean_capa',
            #                                 spread_type_list=['capa_spreads'])
    ...
    def create_spreads(self, prediction_dict,
        ...
        method_mapping = {
                ...
                "single_markets": {
                    "method": self.compute_single_markets_scenarios,
                    "params": ["curve_pred", "base_products", "columns_dict"],
                }
    ...
    def get_scenario_columns(data_dict):
        ...
        columns_dict = {
            ...
            'perc_col': [a for a in temp.columns if ('_perc' in a) and ('_perc_' not in a)]
        }
        del temp
    ...
    def calculate_country_spreads(self, data_dict, base_products,
                                  selected_columns_dict, include_periods_info=True):
        spreads_dict = {}
                ...
                for level in ["base", "peak"]:
                    ...
                    for scenario_type, filtered_df in base_peak_spreads[level].items():
                        ...
                        for product in base_products:
                            start_date, end_date = self._get_product_period(fcst_date, product)
    ...
    def calculate_period_spreads(self,data_dict, base_products, selected_columns_dict,
                                 include_periods_info=True):
                ...
                for scenario_type, columns in selected_columns_dict.items():
                    ...
                    for i, product1 in enumerate(base_products):
                        for product2 in base_products[i + 1:]:
                            ...
                            start_date1, end_date1 = self._get_product_period(fcst_date, product1)
                            start_date2, end_date2 = self._get_product_period(fcst_date, product2)
                            ...
                            if "_sim" in scenario_type:
                                ...
                                base_stats = {
                                    "mean": (base_period_data1.mean(axis=1).mean() - 
                                             base_period_data2.mean(axis=1).mean()),
                                    "median": (base_period_data1.median(axis=1).mean() - 
                                               base_period_data2.median(axis=1).mean()),
                                    "90th": (base_period_data1.quantile(0.9, axis=1).mean() - 
                                               base_period_data2.quantile(0.9, axis=1).mean()),
                                    "10th": (base_period_data1.quantile(0.1, axis=1).mean() - 
                                               base_period_data2.quantile(0.1, axis=1).mean()),
                                }
                                ...
                                peak_stats = {
                                    "mean": (peak_period_data1.mean(axis=1).mean()
                                             - peak_period_data2.mean(axis=1).mean()),
                                    "median": (peak_period_data1.median(axis=1).mean() - 
                                               peak_period_data2.median(axis=1).mean()),
                                    "90th": (peak_period_data1.quantile(0.9, axis=1).mean() - 
                                               peak_period_data2.quantile(0.9, axis=1).mean()),
                                    "10th": (peak_period_data1.quantile(0.1, axis=1).mean() - 
                                               peak_period_data2.quantile(0.1, axis=1).mean()),
                                }
    ...
    def calculate_base_peak_spreads(self,data_dict, base_products,
                                    selected_columns_dict, include_periods_info=True):
        spreads_dict = {}
        periods_info = {} if include_periods_info else None
    ...
    def calculate_clean_spark_spreads(self, data_dict, base_products, selected_columns_dict,
                                      fuel_data, include_periods_info=True):
        css_dict = {}
        periods_info = {} if include_periods_info else None
    ...
    def calculate_capa_spreads(self,data_dict, base_products,
                               selected_columns_dict, selected_spreads=None,
                               include_periods_info=True):
        """
    ...
    def compute_single_markets_scenarios(
        self,
        data_dict: Dict[pd.Timestamp, Dict[str, pd.DataFrame]],
        base_products: List[str],
        selected_columns_dict: Dict[str, List[str]],
        include_periods_info: bool = True
    ) -> Union[Dict, Tuple[Dict, Dict]]:
        """
        Compute scenarios for each country, separating results into base and peak levels
        and aggregating statistics over specified periods.
    
        Args:
            data_dict (Dict[pd.Timestamp, Dict[str, pd.DataFrame]]): Nested dictionary where
                the first level is forecast dates and the second level is country data as DataFrames.
            base_products (List[str]): List of product strings defining time periods 
                (e.g., "W_1", "M_2", "Wknd_1", "D_3").
            selected_columns_dict (Dict[str, List[str]]): Dictionary with scenario types as keys
                and lists of relevant columns as values.
            include_periods_info (bool): Whether to include period start/end dates for each product.
    
        Returns:
            Union[Dict, Tuple[Dict, Dict]]: A dictionary of scenarios, optionally with period info.
        """
        scenarios_dict = {}
        periods_info = {} if include_periods_info else None
    
        for fcst_date, countries_data in data_dict.items():
            scenarios_dict[fcst_date] = {}
            if include_periods_info:
                periods_info[fcst_date] = {}
    
            for country, df in countries_data.items():
                filtered_scenarios = {"base": {}, "peak": {}}
    
                for scenario_type, columns in selected_columns_dict.items():
                    filtered_df = df[columns]
    
                    # Base: Full period
                    filtered_scenarios["base"][scenario_type] = filtered_df
    
                    # Peak: Weekdays and specified hours
                    peak_filtered_df = filtered_df[
                        (filtered_df.index.weekday < 5)  # Weekdays (Monday to Friday)
                    ].between_time("08:00", "20:00")  # Between 08:00 and 20:00
                    filtered_scenarios["peak"][scenario_type] = peak_filtered_df
    
                # Aggregating scenarios based on the base_products list
                aggregated_scenarios = {"base": {}, "peak": {}}
                for level in ["base", "peak"]:
                    for scenario_type, filtered_df in filtered_scenarios[level].items():
                        scenario_stats = {}
                        for product in base_products:
                            start_date, end_date = self._get_product_period(fcst_date, product)
                        ...
                        # Store the aggregated statistics for the current scenario
                        aggregated_scenarios[level][scenario_type] = scenario_df
    
                # Store aggregated scenarios for the country
                scenarios_dict[fcst_date][country] = aggregated_scenarios
    
        out_dict = {"single_prices": scenarios_dict}
