...
class ExternalFundScenarios(scenarios):
    ...
    def get_historical_scenarios(self, scen_list, fcst_date):
        if self.fund_type in ['ResidualDemand']:
            ...
            wind_inst.unique_markets_list = ['de', 'fr', 'be', 'nl', 'at']
            ...
            solar_inst.unique_markets_list = ['de', 'fr', 'be', 'nl', 'at']
        ...
        normal_curve, curve_normalized = normal_curve.align(curve_normalized, join='inner',axis=0)
        normal_curve, res_normal_curve = normal_curve.align(res_normal_curve, join='inner', axis=0)
        
        # Sample scenarios with OU process using GPU
        percentiles_dict = self.sample_scenarios_with_ou(monthly_scen_dict,
                                                         tau_dict,
                                                         variance_dict,                                                         
                                                         curve_normalized,
        ...
        normal_curve_arr = normal_curve.drop(['month'],axis=1).values
        ...
        scen_curves_arr = self.map_scenarios_to_rld(percentiles_arr, normal_curve_arr, res_normal_curve_arr)
        
        scen_curves_dict = {a: pd.DataFrame(scen_curves_arr[i],columns=normal_curve.drop(['month'],axis=1).columns,
                                            index=normal_curve.index)
