    ...
    def get_curve(self, date_shift=-1):
        ...
        for market, market_df in self.fut_matrix.items():
            ...
            if not market in self.data_curve:
                ...
                year_list = [curve_last_date.year + int(a.split('_')[1])-1
                                if curve_last_date < dt.datetime(curve_last_date.year,12,1) 
                                else curve_last_date.year + int(a.split('_')[1])
                ...
                curve = curve.resample('h').bfill().bfill()
