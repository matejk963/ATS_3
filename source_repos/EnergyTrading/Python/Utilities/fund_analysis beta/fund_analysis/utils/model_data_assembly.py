...
class ModelDataAssembly:
    ...
    def collect_data(self, selected_data_type=None):
        ...
        for predictor in self.predictors:
            ...
            if selected_data_type is None:
                ...
                if not self._has_data_for_type(inst, 'perc'):
                    self._process_perc_curve_data(inst, predictor)
            elif selected_data_type in ['curve']:
                self._process_curve_data(inst, predictor, gas_curve, gas_da)
            elif selected_data_type in ['da']:
                self._process_da_data(inst, predictor)
            elif selected_data_type in ['perc']:
                self._process_perc_curve_data(inst, predictor)
    ...
    def _process_perc_curve_data(self, inst, predictor):
        """Processes 'da' data type for the given predictor."""    
        if predictor in ['ResidualDemand']:
            inst.get_perc_curve()
            self.data_dict[predictor]['perc'] = copy.deepcopy(inst.perc_curve)
    ...
    def _has_data_for_type(self, inst, data_type):
        ...
        if data_type == 'da':
            ...
            return hasattr(inst, 'data_perc_curve') and bool(inst.data_curve)
        return False
    ...
    def create_base_data(self, normalized=False, selected_data_type=None):
        if selected_data_type is None:
            if normalized:
                ...
                data_type_list = ['da', 'curve', 'perc']
        else:
            if isinstance(selected_data_type, str):
                data_type_list = [selected_data_type]
            else:
                data_type_list = selected_data_type
            
        # Collect data
        for selected_type in data_type_list:
            self.collect_data(selected_data_type=selected_type)
        ...
        for data_type in data_type_list:  # Adjust the data types as needed
            ...
            for predictor in self.predictors:
                if data_type in ['perc']:
                    continue
                if predictor not in ['ResidualDemand', 'Hydro']:
                    if normalized:
                        continue
        ...
        if 'perc' in data_type_list:
            new_dict['perc'] = copy.deepcopy(data_dict['ResidualDemand']['perc'])
    ...
    def features_method_mapping(self, feature_type: str):
        method_map = {
            ...
            'gmc': self.create_gmc,
            'cmc': self.create_cmc,
            'dt_atr': self.create_datetime_atributes
        }
    ...
    def create_datetime_atributes(self, df: pd.DataFrame):
