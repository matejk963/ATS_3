...
def coal_data_to_db(base_date=dt.datetime(2024,6,1),
                    fut_periods=12):
    db = Database()
    start_date, end_date = get_dates(db, 'atw', base_date)
    rics_list = [[f'ATWMc{str(a+1)}' for a in range(fut_periods-10,
                                                    fut_periods)]
    ...
    if len(data)==0:
        ...
        data.to_sql(name="stage_atw" ,
                    schema='futures',
                    con=db.connection_string,
                    if_exists='replace', index=False)
        db.merge_from_staging_to_prod('futures', "atw")
        
        
coal_data_to_db(base_date=dt.datetime(2019,1,1), fut_periods=60)