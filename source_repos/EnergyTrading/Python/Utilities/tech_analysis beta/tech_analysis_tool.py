...
class TechAnalysisTool:
    ...
    def compute_sheet_name(self, sheet_name):
        ...
        if sheet_name is None or sheet_name not in self.sheets:
            print(f"[ERROR] compute_sheet_name called with invalid sheet_name: {sheet_name}")
            return sheet_name or "Sheet 1"
    ...
    def open_leg_popup(self, sheet_name, chart_type):
        ...
        if self.current_sheet is None:
            print("[ERROR] open_leg_popup called but self.current_sheet is None. Aborting popup.")
            return
