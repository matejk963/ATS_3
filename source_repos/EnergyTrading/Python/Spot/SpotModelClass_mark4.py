...
class PowerModel:
    ...
    @property
    def spread_type_mapping(self):        
        func_dict = {
            'country_srpead': lambda pm_inst: pm_inst.get_country_spread(),
            'calendar_spread' : lambda pm_inst: pm_inst.get_calendar_spread(),
            'css' : lambda pm_inst: pm_inst.get_css(),
            'base_peak': lambda pm_inst: pm_inst.get_base_peak_spread()
        }
        return func_dict
    ...
    def get_av_cap_fcst(self, markets=['de', 'fr'], source='db'):
        ...
        if hasattr(self, 'av_cap_for_fcst_dict'):
            ...
            for market in markets:
                self.av_cap_for_fcst_dict[market] = self.am_inst.get_data_fcst_raw(market,source=source)
    ...
    def assemble_fcst_predictors(self, train_dict):
        ...
        fcst_data['month'] = fcst_data.index.month
        fcst_data['hour'] = fcst_data.index.hour
            ...
            for scen_type, scen_type_dict in fcst_dict.items():
                ...
                for model_name, models_dict in self.fitted_models_dict.items():
                    ...
                    for market in list(train_dict):
                        ...
                        residuals, res_std = self.residuals_adjustment(train_dict, market, y_train_pred, fcst_data)
                        fuels_costs_series = self.fcst_data['ttf_da'] + self.fcst_data['eua'] * 0.2
                        
                        additional_data_list = [res_std, fuels_costs_series, fcst_data]
                        
                        def process_forecast(i, predictors_data, raw_df):
                            ...
                            fuels = raw_df['ttf_da'].values + raw_df['eua'].values * 0.2
                            forecast_nominal = pd.DataFrame((forecast.values * fuels.reshape(-1,1)),
                                                            index=self.fcst_data.index,
                                                            columns=[i])
                            return forecast, forecast_nominal
                        
                        results = Parallel(n_jobs=-1)(
                            delayed(process_forecast)(i, predictors_data, raw_df)
                            for i, predictors_data, raw_df in zip(range(len(transformed_dfs)), transformed_dfs, changed_dfs)
                        )
                        
                        for forecast, forecast_nominal in results:
                            final_data_dict['ghr'] = pd.concat([final_data_dict['ghr'], forecast], axis=1)
                            final_data_dict['nominal'] = pd.concat([final_data_dict['nominal'], forecast_nominal], axis=1)
                            
                        for i, data_type in enumerate(['ghr', 'nominal', 'std', 'fuels_costs', 'fcst_data']):
    ...
    @staticmethod
    def create_country_spread(input_dict):
        new_dict = {}
    ...
    def get_country_spread(self,model_params_dict,
        ...
        spread_dict = self.create_country_spread(prod_dict)
    ...
    def get_calendar_spread(self,model_params_dict,
        ...
        for fcst_date, fcst_dict in prod_dict.items():
            ...
            for country, country_dict in fcst_dict.items():
                calendar_spread_dict[fcst_date][country] = self.create_calendar_spread(country_dict)
    ...
    @staticmethod
    def create_calendar_spread(input_dict):
        mean_df = input_dict['mean']
        std_df = input_dict['std']
        fuels_df = input_dict['fuels']

        # Reset the index and prepare tuples for MultiIndex
        mean_df = mean_df.reset_index()
        std_df = std_df.reset_index()
        fuels_df = fuels_df.reset_index()

        row_tuples = [(p, y, d) for p, y, d in zip(fuels_df['product'], fuels_df['year'], fuels_df['delivery'])]
        ...
        # Iterate over each column in the mean DataFrame
        for col in mean_df.columns[3:]:  # Assuming the first column is the index
            # Compute the difference matrix for this column (row - column)
            column_values = mean_df[col].values
        ...
        # Compute the standard deviation spread
        for col in std_df.columns[3:]:  # Assuming the first column is the index
            column_values = std_df[col].values
        ...
        if mean_dfs and std_dfs:
            ...
            mean_cols = [col for col in combined_mean_df.columns if 'Difference_' in col]
            std_cols = [col for col in combined_std_df.columns if 'Std_Spread_' in col]
    ...
    def get_fcst_for_range(self, fcst_range, model_params_dict,
                           spread_type, nominal=True):
        tot_curves_dict = {}
        self.data_collection = {}
