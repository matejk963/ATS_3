...
class PowerModel:
    ...
    def assemble_fcst_predictors(self, train_dict):
        ...
        """
        if self.pivot_date == dt.datetime(2023,8,7):
            print('708')
        self.fcst_data = {}
    ...
    def create_fund_curve(self, data, fund, market):
        if self.pivot_date == dt.datetime(2024,6,10):
            print('713')
    ...
    def transform_and_adjust(self, base_df, alt_df=None, rld_columns=None, target_columns=None):
            ...
            for scen_type, scen_type_dict in fcst_dict.items():
                ...
                if scen_type_dict:
                    transformed_dfs = Parallel(n_jobs=-1)(
                        delayed(self.transform_and_adjust)(fcst_data, df, rld_columns, target_columns) 
                ...
                for model_name, models_dict in self.fitted_models_dict.items():
                    ...
                    for market in list(train_dict):
                        ...
                        for i, predictors_data in enumerate(transformed_dfs):
        
                            forecast = pd.DataFrame(fitted_model.predict(predictors_data),
                            ...
                            forecast_nominal = pd.DataFrame((forecast[i].values*
                                                             fuels_costs_series.values),
                                                            index=self.fcst_data.index,
                                                            columns=[i])
                            if ghr_df.empty:
                                ghr_df = forecast.copy()
                            else:
                                ghr_df = pd.concat([ghr_df, forecast], axis=1)
                            if nominal_df.empty:
                                nominal_df = forecast_nominal.copy()
                            else:
                                nominal_df = pd.concat([nominal_df, forecast_nominal], axis=1)
                            
                            
                        self.fcst_curve_dict[fcst_date][scen_type][model_name][market]['ghr'] = ghr_df
                        self.fcst_curve_dict[fcst_date][scen_type][model_name][market]['nominal'] = nominal_df
                        self.fcst_curve_dict[fcst_date][scen_type][model_name][market]['fcst_data'] = fcst_data
    ...
    def residuals_adjustment(self, train_dict, market,
        ...
        features_index = [train_dict[market][0].index(feature) for feature in selected_features]
        ...
        for hour in range(23):
            ...
            scaler = PowerTransformer()
            X_train_scaled = scaler.fit_transform(X_train[hours_mask][:, features_index])
                ...
                for key1, subdict1 in nested_dict[key2][key5].items():
                    ...
                    for key4, df in subdict1.items():                        
                        ...
                        for mask_vars in masks:
                            ...
                            if delivery == 'base':
                                mean_series = masked_df.mean(axis=0)
                            elif delivery == 'peak':
                            ...
                            combined_means.append(mean_series)
                            index_tuples.append((product, year, delivery))
    
                        # Combine all means into a DataFrame with a multi-level index
                        constructed_df = pd.DataFrame(combined_means, index=pd.MultiIndex.from_tuples(index_tuples, names=['product', 'year', 'delivery']))
    
                        if key1 not in new_dict:
                            new_dict[key1] = {}
    
                        new_dict[key1][key4] = constructed_df
    
        return new_dict
    # def apply_masks_and_compute_means(nested_dict, key2, key5, masks):
    #     new_dict = {}
    ...
    #                     combined_means = []
    #                     index_tuples = []
    #                     for mask_vars, mask_func in masks:
    #                         masked_df = df[mask_func(df)]
    #                         mean_series = masked_df.mean(axis=0)
    #                         combined_means.append(mean_series)
    #                         index_tuples.append(mask_vars)
    ...
    def get_css(self,model_params_dict,
        ...
        prod_dict = self.get_product_forecast(model_params_dict,
                                 ...
                                 scenario_type,
                                 fcst_value_type)
        fcst_dict = self.get_product_forecast(model_params_dict,
                                 aux_dict,
                                 scenario_type,
                                 'fcst_data')
        ...
        for date, date_dict in prod_dict.items():
            ...
            for market, market_df in date_dict.items():
                fuels_data = fcst_dict[date][market][['ttf_da', 'eua']]
                final_df = pd.DataFrame()
                for i, col in enumerate(market_df.columns):
                    aux = pd.concat([market_df[col], fuels_data], axis=1)                
                    aux['css_' + str(i)] = aux[col] - 2*(aux['ttf_da'] + aux['eua']*0.2)
                    if final_df.empty:
                        final_df = aux.copy()
                    else:
                        final_df = pd.concat([final_df, aux[['css_' + str(i)]]], axis=1)
                css_dict[date][market] = final_df
    ...
    def create_calendar_spread(input_df):
        ...
        for col in input_df.columns[3:]:  # Assuming the first three columns are not numerical
            ...
            all_upper_dfs.append(upper_df)
    
        # Concatenate all upper DataFrames along columns
        combined_upper_df = pd.concat(all_upper_dfs, axis=1)
        combined_upper_df = combined_upper_df.loc[:,~combined_upper_df.columns.duplicated()]  # Remove duplicate index columns if any
    
        return combined_upper_df
    ...
    # def get_product_forecast(self, train_dict,
    #                          model_params_dict,
    #                          process_models='mean'):
    #     if not hasattr(self, 'fcst_dict'):
    #         self.get_curve_forecast(train_dict,model_params_dict)
    #     mask_list = []
    #     data_index = self.fcst_data.index
    #     peak_index = ((data_index.hour>7)&(data_index.hour<20)&(data_index.weekday<5))
    #     unique_product, unique_del, unique_year = self.drop_duplicates_from_multiple_lists(self.params_dict['product_list'],
    #                                                                                        self.params_dict['delivery_list'],
    #                                                                                        self.params_dict['year_list'])
    #     for product, delivery, year in zip(unique_product, unique_del, unique_year):
    #         del_start, del_end = self.get_start_end_product_date(product,delivery,year)
    #         mask = ((data_index>=del_start)&(data_index<=del_end))
    #         if delivery.lower() in 'base':
    #             del_list = [True for a in mask]
    #         else:
    #             del_list = peak_index
                
    #         mask_list.append([a and b for a,b in zip(mask,del_list)])
            
    #     aux_dict = defaultdict(list)
    #     for model, model_dict in self.fcst_curve_dict.items():
    #         for fcst_type, df in model_dict.items():
    #             aux_dict[fcst_type].append(df)
    #     curve_dict = {}
    #     for fcst_type, dfs in aux_dict.items():
    #         sum_df = dfs[0].copy()
    #         for df in dfs[1:]:
    #             sum_df += df
    #         avg_df = sum_df/len(dfs)
    #         curve_dict[fcst_type] = avg_df.copy()
            
    #     prod_dict = {}

    #     for fcst_type, fcst_df in curve_dict.items():
    #         aux_prod_curve = pd.DataFrame()
    #         for mask, product, year, delivery in zip(mask_list,unique_product,
    #                                                  unique_del, unique_year):
    #             aux = fcst_df[mask].copy()
    #             aux_df = pd.DataFrame([aux.mean()],index=pd.MultiIndex.from_product([[product], [year], [delivery]], names=['product', 'year', 'delivery']))
    #             if aux_prod_curve.empty:
    #                 aux_prod_curve = aux_df.copy()
    #             else:
    #                 aux_prod_curve = pd.concat([aux_prod_curve,
    #                                             aux_df])
    #         prod_dict[fcst_type] = aux_prod_curve.copy()
    #     return prod_dict
