...
class AvCapMonitor:
    ...
    def get_data_fcst_raw(self, market, source='db'):
        if source in ['db']:
...
class RldMonitor:
    ...
    def get_raw_data(self, market='de', source='local', long=True,
                     ...
                     normal=False, normal_parts=False,
                     data_fund='rld'):
        ...
        if source in ['db']:
            ...
            if monthly:
                ...
                for fund in ['CON', 'Wind', 'Solar']:
                    schema_name = 'FUND_' + fund + '_mnd'
                    table_name = country + '_00'
                    aux = self.get_fund_data_db(schema_name, table_name)
                ...
                df = pd.DataFrame()
                for fund in ['CON', 'Wind', 'Solar']:
                    schema_name = 'FUND_' + fund + '_normal'
                    table_name = country
                    aux = self.get_fund_data_db(schema_name, table_name)
                ...
                if normal_parts:
                    ...
                    return df
            elif data_fund.lower() in ['inf']:
                df = pd.DataFrame()
                fund = data_fund
                schema_name = 'FUND_' + fund
                table_name = country
                aux = self.get_fund_data_db(schema_name, table_name)
                aux.columns = ['value_date', fund]
                aux = aux.loc[~aux['value_date'].duplicated()].copy()
                if df.empty:
                    ...
                    df = df.merge(aux, on=['value_date'],how='inner')
                df = df.set_index('value_date').resample('h').ffill()
                df.reset_index
                return df
            else:
                schema_name = 'FUND_ResidualDemand'
    ...
    def get_raw_data_normalized(self, market='de', source='local', long=True,
        ...
        if not smooth:
            ...
            df['daily_avg'] = df.groupby(pd.Grouper(key='value_date', freq='D'))['rld_norm'].transform('mean')
            df['rld'] = df['rld']/df['daily_avg']
    ...
    def get_rld_curve(self, market='de', source='local', fut_periods=8760,
        ...
        if normalize:
            ...
            normal_curve = self.get_raw_data(market=market, source='local', normal=True)
            normal_curve['rld'] = normal_curve['rld'] * (1 - normal_correction_factor/100)
            # Process them to matrix
            short_matrix = self.data_matrix(short_fcst)
            monthly_matrix = self.data_matrix(monthly_fcst)
            additional_columns = [col for col in monthly_matrix.columns
    ...
    def get_mask(self, filter_values, df, df_act, market, types='all'):
        ...
        for i, key in enumerate(['weekday', 'hour', 'hist_position']):   
        ...
        for col in curve.columns:
            ...
            for mask_type in conditions:
                if mask_type in ['month']:
                    ...
                    quantile = find_value_quantile(val, val_ser, 20)
    ...
    def get_scenarios_for_hour(self, market, fwd_day, curve_date,
        ...
        for i, hour in enumerate(values_df_adj.columns):
            ...
            masks_dict = self.get_mask(values,change_matrix_adj, normalized_matrix_smooth_adj, market)
            dist_list = []
            dist_params_list = []
            data_list = []
    ...
    def get_timeseries_scenarios(self, market, days_fwd_list, source,
        ...
        transition_matrix = self.calculate_transition_matrix(self.get_da_data(quantiles), 20)
        ...
        quantile_edge = self.get_quantile_edges(normalized_matrix_smooth_adj, 20)
        ...
        for fwd_day in days_fwd_list:
            ...
            for i, hour in enumerate(hours_index):
                ...
                dist_states = np.unique([a if a<30 else 29 for a in dist_states])
                ...
                for run in range(runs):
                    ...
                    if i == 0:
                        ...
                        try:
                            aux_samples_adj = self.sample_from_truncated_kde(data=aux_samples,
                        ...
                        if len(aux_samples_adj_nominal) == 0:
                            print('2351')
                ...
                if max(run_list)-min(run_list) < 5000:
                    print('2336')
            end_time1 = time.time()
