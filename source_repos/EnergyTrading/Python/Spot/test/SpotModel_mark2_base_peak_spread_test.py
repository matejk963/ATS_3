...
products = ['M_1', 'M_2', 'M_3', 'Q_1', 'Q_2', 'Q_3']

params_dict['product_list'] = products*2

params_dict['market_list'] = ['de'] * len(products) * 2

params_dict['delivery_list'] = ['base'] * len(products) + ['peak'] * len(products)
...
scen_dict2 = {dt.datetime(2024,5,27):{'liq':{}}}



prod_dict = pm_inst.get_product_forecast(model_params_dict, scen_dict2)

country_dict = prod_dict[list(prod_dict)[0]]

de_df = country_dict['de']

pivot_df = de_df.pivot_table(index=['product', 'year'], columns='delivery', values=0, aggfunc='sum')

# Calculate the difference
pivot_df[0] = pivot_df['base'] - pivot_df['peak']
pivot_df = pivot_df.drop(['base', 'peak'],axis=1)
