...
file_paths = [
    ...
    r"Z:\Data\Scenarios\scen_dict_20240401_20240601.pickle"
    ]
    ...
    for scen_date, scen_data in scen_dict.items():
        ...
        for date, date_dict in scen_fcst_dict.items():
            for market, market_dict in date_dict.items():
                for data_type, market_df in market_dict.items():
        ...
        folder_path = r'Z:\Data\Spot\Model\Backtest\ModelPrices\MMSpot\MM weekly price scenarios 20240401_20240601'
        
        # start_end_dates = file_path.split('\\')[-1].split('_')
        # start_date = start_end_dates[-2]
        # end_date = start_end_dates[-1].split('.')[0]
        
        file_name = 'de_w1_w2_' + scen_date.strftime('%Y%m%d') + '_3.pickle'
        file_name_prod_dict = 'de_w1_w2_prod_dict_' + scen_date.strftime('%Y%m%d') + '_3.pickle'
        file_path = os.path.join(folder_path, file_name)
        file_path_prod_dict = os.path.join(folder_path, file_name_prod_dict)
        
        with open(file_path, 'wb') as file2:
            pickle.dump(scen_fcst_dict, file2)
        with open(file_path_prod_dict, 'wb') as file2:
            pickle.dump(pm_inst.prod_dict, file2)
