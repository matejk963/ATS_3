...
class StrategyStats(AdditionalTypeABC):
    ...
    def net_clip_position(self):
        trade_leg_dict = self.price_vol_trade_leg_dict
        return {k: (v['buy'].quantity - v['sell'].quantity) / self.leg_clip_dict[k] for k, v in trade_leg_dict.items()}

    @property
    def total_net_position(self):
        return sum([v for v in self.net_clip_position.values()])
    ...
    def is_legs_price(self):
        return all(x is not None for subdict in self.price_leg_dict.values() for x in subdict.values())
    ...
    def is_ref_price(self):
        if self.ref_instkey is None:
            ...
            return all(x is not None for x in self.price_ref_dict.values())
    ...
    def bo_legs_dict(self):
        return {k: calc_bo(subdict['buy'], subdict['sell']) for k, subdict in self.price_leg_dict.items()}
