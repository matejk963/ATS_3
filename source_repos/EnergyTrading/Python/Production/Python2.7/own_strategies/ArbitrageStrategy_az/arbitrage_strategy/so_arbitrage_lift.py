...
class LiftOrder(SYB.SyntheticOrderBase):
    ...
    def get_open_position(self, localview, other_view):
        ...
        if net_open_position > 0:
            ...
            if price_own and price_own<=round_to_tick(price_last+self.margin,self.tick_size):
                price=price_own
                broker_id=broker_own
            elif price_mm:
                if last_order_price_own and round_to_tick(abs(price_mm+self.tick_size -last_order_price_own),self.tick_size)<=self.idle_threshold:
                    price=last_order_price_own
                else:
                    price=round_to_tick(price_mm+self.tick_size,self.tick_size)
            ...
            if price_own and price_last and round_to_tick(price_own+self.margin,self.tick_size)>=price_last:
                price=price_own
                broker_id=broker_own
    ...
    def act(self, localview, additional_views, timestamp):
        ...
        if open_position > 0 and price>0: # and (bid_not_balancing or ask_not_balancing)
            #logging the details
            ...
            # logging the details
            log.debug("[{}] [{}] [{}] LIFT-SO-ACT-REMOVE-SLOT: [OP: {},  BNB: {}, ANB: {}]".format(
