...
class CustomStrategy(STRAT.Strategy):
    ...
    def custom_act_for_product(self, log_data, product, timestamp, *args, **kwargs):
        ...
        buy_slot = STRAT.PositionSlot("buy", COMMON.Direction.buy, max(rest, 0), price_buy)
        sell_slot = STRAT.PositionSlot("sell", COMMON.Direction.sell, max(-rest, 0), price_sell)

        results = self.place_slots(log_data, product, timestamp, self.delivery_area_id, [buy_slot], ["placement_info"])

        results = self.place_slots(log_data, product, timestamp, self.delivery_area_id, [sell_slot], ["placement_info"])
