...
class ArbitrageOrder(SOB.SyntheticOrderBase):
    ...
    lift_instrument_id = CONF.SyntheticOrderConfigField(caption="lift_instrument_id",
    ...
    def act(self, localview, additional_views, timestamp):
        ...
        view_ttf = additional_views.get_configured_view("view_ttf", localview.product_id)
        view_ncg = additional_views.get_configured_view("view_ncg", localview.product_id)

        print("view_ncg: %s : %s" % (view_ncg.current_front_price(COMMON.Direction.buy, self.broker_id), COMMON.Direction.buy))
        print("view_ncg: %s : %s" % (view_ncg.current_front_price(COMMON.Direction.sell, self.broker_id), COMMON.Direction.sell))

        print("view_ttf: %s : %s" % (view_ttf.current_front_price(COMMON.Direction.buy, self.broker_id), COMMON.Direction.buy))
        print("view_ttf: %s : %s" % (view_ttf.current_front_price(COMMON.Direction.sell, self.broker_id), COMMON.Direction.sell))
