...
def load_json_data(asset_filename):
    with open(path(asset_filename), "r") as f:
        return json.load(f)
...
TRAYPORT_EXCHANGE_FEED_MAY_4th = common_workshop_asset(os.path.join("ttf", "2021-05-04_COMPLETE_first_10000msg.zip"))

TRAYPORT_INIT_FILES_MAY_4th = common_workshop_asset(os.path.join("ttf", "init_files_2021-05-04.zip"))
...
class GasStrategyBacktest(unittest.TestCase):
    def test_example_simulation(self):
        ...
        simulator = SIM.Simulator(strategy_messages,
                                  feed_paths=[TRAYPORT_EXCHANGE_FEED_MAY_4th],
                                  simulated_exchanges=(COMMON.Exchange.trayport, ),
                                  use_persistence=True,  # set this to True to use MongoDB
                                  trayport_init_directory=TRAYPORT_INIT_FILES_MAY_4th,
