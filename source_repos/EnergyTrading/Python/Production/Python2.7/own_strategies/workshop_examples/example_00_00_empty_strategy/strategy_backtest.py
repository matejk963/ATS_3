...
class PowerStrategyBacktest(unittest.TestCase):
    def test_01(self):
        ...
        # timeseries will be entered for values from starttime to endtime
        start_time_cet = datetime.datetime(2021, 05, 17, 8, 0, 0)
        end_time_cet = datetime.datetime(2021, 05, 17, 9, 0, 0)
        transfer_time_cet = datetime.datetime(2021, 05, 17, 5, 55, 0)

        # convert cet time to utc, since the strategy takes utc datetimes
        start_time = ALCU.cet_dt2utc_dt(start_time_cet)
        end_time = ALCU.cet_dt2utc_dt(end_time_cet)
        transfer_time = ALCU.cet_dt2utc_dt(transfer_time_cet)
