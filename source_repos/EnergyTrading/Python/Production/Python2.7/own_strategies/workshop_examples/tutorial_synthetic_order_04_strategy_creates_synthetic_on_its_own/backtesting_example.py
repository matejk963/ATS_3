...
class StrategyConfigurationTest(unittest.TestCase):
    def test(self):
        ...
        strategy_config.update(
            dict(
                so_products=[COMMON.SequenceId.gas_prompt + "_" + COMMON.GasPromptItemId.within_day,  # 10000302_1
                             COMMON.SequenceId.gas_prompt + "_" + COMMON.GasPromptItemId.day_ahead],  # 10000302_3
                so_positions=[100, 400]
            )
        )
        ...
        limits_payload = {
            "limits_per_sequence": {"maximum_purchase_volume": {COMMON.SequenceId.gas_prompt: 1000},
                                    "minimum_sales_price": {COMMON.SequenceId.gas_prompt: -1000},
                                    "maximum_sales_volume": {COMMON.SequenceId.gas_prompt: 1000},
                                    "maximum_purchase_price": {COMMON.SequenceId.gas_prompt: 1000}
                                    },
        }

        limits_setup_message = SBU.create_per_products_limit_message(timestamp=dt_to_ts(DT(2021, 6, 15, 12, 2, 1)),
                                                                     strategy_id=strategy_id,
                                                                     limits_dict=limits_payload)

        simulator = SIM.Simulator(
            [strategy_insert_message, limits_setup_message, strategy_load_so_message, strategy_activate_message],
