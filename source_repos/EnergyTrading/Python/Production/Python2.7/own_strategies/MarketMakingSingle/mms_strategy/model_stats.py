...
log = logging.getLogger("market_making_single.model_statistics")
...
class ModelStats(object):
    ...
    def update_models(self, model_type, model_params):
        ...
        if model_type is None:
            ...
            self.model_dict['spread'] = EmaModel(model_params[0], 0, model_params[1], False)
        elif self.model_type == 'MSTD_x':
            self.model_dict['spread'] = MstdModel(model_params[0], 30, 2., 0, model_params[1], 10, False, False)
        elif self.model_type == 'MSTD_t':
            self.model_dict['spread'] = MstdModel(model_params[0], 30, 2., 0, model_params[1], 10, True, False)
        elif model_type == 'MID':
            self.model_dict['spread'] = MidModel(0, model_params[1])
        else:
            raise ValueError("Unknown model type in ModelStats: %s." % model_type)
    ...
    def reset(self):
        if self.model_type is None:
            ...
            self.model_dict['spread'] = MstdModel(tau, 30, 2., 0, tol, burn, False, False)
        elif self.model_type == 'MSTD_t':
            tau = self.model_params[0]
            tol = self.model_params[1]
            burn = self.model_dict['spread'].burn
            self.model_dict['spread'] = MstdModel(tau, 30, 2., 0, tol, burn, True, False)
...
class MstdModel(ModelClass):
    ...
    def __init__(self, tau, N, p=2., value0=0, tol=.025, burn=10, time_model=False, is_init=False):
        super(MstdModel, self).__init__(tol)
        self.model = MSTD(tau / 2, N, p, value0)
        if time_model:
            self.dt = self.dt_1
        else:
            self.dt = self.dt_x
    ...
    def initialize(self, value0):
        param_list = [self.model.tau, self.model._ma[0].n, self.model.p, value0]
