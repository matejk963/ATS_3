...
from mms_strategy.constants import (BROKER_ID, GAS_YEARS, GAS_YR22_ITEM_ID, TTF_ICE, TTF_PRODUCT_ID,
...
def create_feed(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        ...
        (GAS_YEARS, TTF_ICE, GAS_YR22_ITEM_ID, BROKER_ID, 52, COMMON.Direction.sell),
        # Power: item id 19 => 2022 (JAN22-JAN23), # item id 20 => 2023
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 47, COMMON.Direction.buy),
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 57, COMMON.Direction.sell),
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
...
class SynthStratConfigTest(unittest.TestCase):
    def setUp(self):
        ...
        self.config = StrategyConfiguration(
            ...
            instrument_ids=[ITALY_BASELOAD, TTF_ICE, TTF_ICE],  # can only place 2 instrument ids here
            strategies_folder=EXAMPLE_FOLDER,
            caption="MarketMaking Synth Strategy",
            package_name="mms_strategy",
            product_ids=[ITALY_PRODUCT_ID, TTF_PRODUCT_ID, TTF_PRODUCT_ID],
            leg_clip=1,
