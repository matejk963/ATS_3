...
# list of libraries relying on standard logging module, where we do not force importing fast logging
LIBS_STD_LOGGING = ["asyncio", "unittest"]

# save the original __import__ which is overwritten for logging module
...
def _import(name, *args, **kwargs):
    # info https://stackoverflow.com/questions/9806963/how-to-use-the-import-function-to-import-a-name-from-a-submodule

    # if some module (except defined in LIBS_STD_LOGGING) tries to import logging...
    if name == "logging" and args[0]["__package__"] not in LIBS_STD_LOGGING:
        name = __name__  # in our case autotrader_lib.fast_logging_py3
        return importlib.import_module(name)  # import fast_logging_py3
    return original_import(name, *args, **kwargs)


def make_standard_logging_module():
    """Register a new builtins.__import__ function to make this module the global logging module,
...
class Logger(object):
    ...
    def _prep_compliance_for_entries(cls, current_time):
        ...
        for entry in _entries_json:
            ...
            if not _silence_entries_counter[comp_text]:
                ...
                content_to_write += json.dumps(entry, ensure_ascii=False, default=str) + "\n"
                _counter_json += 1
            else:
                # there should only be 1 timestamp in the Counter anyway
                allowed_time = list(_silence_entries_counter[comp_text].keys())[0]
                if allowed_time > current_time:
                    ...
                    content_to_write += json.dumps(entry, ensure_ascii=False) + "\n"
                    _counter_json += 1
    ...
    def _prep_compliance_for_flushing_thread(cls, current_time):
        ...
        for entry_text in list(_silence_entries_obj):
            allowed_time = list(_silence_entries_counter[entry_text].keys())[0]
            if allowed_time > current_time:
                continue
            elif allowed_time <= current_time:
                ...
                line = json.dumps(_silence_entries_obj[entry_text], ensure_ascii=False)
    ...
    def _log_object(self, level, string, args):
        ...
        if isinstance(string, str):
            if args:
                try:
                    _entries.append(("%s [%s] %s " + string) % ((NOW().isoformat(), self.scope, level) + args))
                except UnicodeDecodeError:
                    args = tuple([element for element in args])
                    _entries.append(("%s [%s] %s " + string) % ((NOW().isoformat(), self.scope, level) + args))
            else:
                _entries.append("{} [{}] {} {}".format(NOW().isoformat(), self.scope, level, string))
        else:
            _entries.append("{} [{}] {} {!r}".format(NOW().isoformat(), self.scope, level, string))
