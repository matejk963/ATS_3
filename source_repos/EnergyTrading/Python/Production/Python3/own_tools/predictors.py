...
def calculate_MACD(market_view, span_short=12, span_long=26):
    # pred_id-s :
    ...
    try:
        ...
        for price, quantity, timestamp in lag_trades[1:]:
            short_ema = calculate_ema(price, short_ema, span=span_short)
            long_ema = calculate_ema(price, long_ema, span=span_long)
