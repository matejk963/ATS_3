...
class MatrixBuilder(strategy.StrategyBase):
    ...
    def get_single_traded_quantity(self, all_range_products, ts):
        ...
        for product in all_range_products:
            ...
            if product.delivery_start <= ts < product.delivery_end:
                # ... iterate all trades of the product
                for trade in product.trades.get(trade_filter=COMMON.TradeFilter.own, portfolio_key=self.strategy_id):
                    slot = trade.tags.get("strategy_slot", "")
                    tradeback = slot.endswith("_tradeback")
    ...
    def build_placement_for_products(
        ...
        for product in tradable_active_products:
            ...
            parts = int(block_length // COMMON.QUARTER)
            segments = int(block_length // duration)
            segment_length = parts / segments
            seconds_since_block_start = product.delivery_start - ts_from

            scale_interval_index = int(seconds_since_block_start // duration)
            # need to add int here, as it is used as an index
            min_idx = int(scale_interval_index * segment_length)
            max_idx = int((scale_interval_index + 1) * segment_length)
            ...
            for ts in range(product.delivery_start, product.delivery_end, COMMON.QUARTER):
                scale_interval_index = int((ts - ts_from) / COMMON.QUARTER)
                # buy
                # 2 options here:
            ...
            buy_price_lower = rd_me(PY2LIB.py2min([PY2LIB.py2min([buy_price_lower, buy_price]), buy_scale_start]))
            buy_price_upper = rd_me(PY2LIB.py2min([PY2LIB.py2max([buy_price_upper, buy_price]), buy_scale_start]))
            sell_price_lower = rd_me(PY2LIB.py2max([PY2LIB.py2min([sell_price_lower, sell_price]), sell_scale_start]))
            sell_price_upper = rd_me(PY2LIB.py2max([PY2LIB.py2max([sell_price_upper, sell_price]), sell_scale_start]))
    ...
    def build_tradeback_placement_for_products(self, tradable_active_products, traded_quantities, maximum_order_book,
        ...
        for product in tradable_active_products:
            ...
            if buy_amounts_product:
                buy_amount = PY2LIB.py2min([elem[0] for elem in buy_amounts_product])
                buy_price = PY2LIB.py2min([elem[1] for elem in buy_amounts_product])
            if sell_amounts_product:
                sell_amount = PY2LIB.py2min([elem[0] for elem in sell_amounts_product])
                sell_price = PY2LIB.py2max([elem[1] for elem in sell_amounts_product])

            # restrict placement by maximum order book setting
            pos = PY2LIB.py2min([rd_me(buy_amount - sell_amount), maximum_order_book])
...
class CustomStrategy(strategy.Strategy, strategy.ProductsFilterMixin, MatrixBuilder):
    ...
    def calc_used_ramps(strategy_closed_positions_before_intraday):
        ...
        # index 0.
        sorted_vals = sorted(list(strategy_closed_positions_before_intraday.items()),
    ...
    def act_for_interval(self, ts_from, ts_until, timestamp):
        duration = int(ts_until - ts_from)
        duration_n_quarters = int(duration // COMMON.QUARTER)
        ...
        a = list(range(ts_from, ts_until + COMMON.QUARTER, COMMON.QUARTER))
