...
class CustomStrategy(SYSB.SyntheticOrderStrategyBase):
    ...
    trade_id_list = []
    opp_slot_dict = {}
    opp_lead_dict = {'buy': {}, 'sell': {}}
    synth_slot_dict = {'lead_bid': {}, 'lead_ask': {}, 'lift': {}}
    initialized = False
    init_orders = True
    reset_bool = False
    model_update_bool = False
    timestamp = None
    ...
    def handle_steering_call(self, payload):
        ...
        elif "model_hist" in payload:
            output_list = self.log_model_list
            # Extra debug log
            self.debug_log('Handle steering call model_hist branch nums' + str(len(self.log_model_list)))
            self.api_export_timeseries({"model_hist": (
                "Statistics of strategy", "MW", "MW", COMMON.HOUR, {self.timestamp: output_list})})
    ...
    def log_model(self):
        if self.extended_logs:
            bid, ask, mid = self.stats.bid_spread, self.stats.ask_spread, self.stats.mid_spread
            # Leg prices
            leg_p = []
            [leg_p.extend([subdict['buy'], subdict['sell']]) for subdict in list(self.stats.price_leg_dict.values())]
            model_p, model_v = self.stats.model_dict['spread'], self.stats.model_dict['std']
            model_count = self.model.model_dict['spread'].count
            timestamp = self.stats.timestamp
            if self.verbose:
                self.debug_log("Log model extended [timestamp]: [{}]: count: [{}//{}]: vals: {}//{}//{}//{}//{}".format(
                    timestamp, model_count, self.log_model_count, model_p, model_v, bid, ask, mid))
            if model_count > self.log_model_count:
                row_list = [timestamp, bid, ask, mid, model_p, model_v, model_count]
                row_list.extend(leg_p)
                self.log_model_list.append(row_list)
                self.log_model_count = model_count
        else:
            pass
    ...
    def on_strategy_configuration_update(self, strategy_json):
        ...
        self.broker1_list = strategy_json["broker1_ids"]
        self.broker2_list = strategy_json["broker2_ids"]
        ...
        self.extended_logs = strategy_json["extended_logs"]
        self.verbose = strategy_json["verbose"]
        self.venue_change_bool = strategy_json["venue_change_bool"]
        ...
        if not self.initialized:
            self.opp_slot_dict = {}
            self.opp_lead_dict = {'buy': {}, 'sell': {}}
            self.synth_slot_dict = {'lead_bid': {}, 'lead_ask': {}, 'lift': {}}
    ...
    def custom_on_order_book_update(self, orders, timestamp):
        # Initialization
        if self.init_orders and self.stats.is_adjusted_model_price:
        ...
        if not tuple_list and not init_bool:
            res = {}
        else:
            for instrument_key, direction in tuple_list:
                if instrument_key.key in self.all_delivery_areas:
                    self.stats.update_strategy_prices(instrument_key, direction,
                                                      self.exchange.products, None)
                    try:
                        # Add to list corresponding synthetic order
                        ord_id_list.append(self.opp_lead_dict[direction][instrument_key.product_id])
            ...
            self.quote()
            # Update model & mtm
            # If model has changed then run all lead orders or if initialize orders
            if self.model_update_bool or init_bool:
                # Run all lead orders
                ord_id_list = []
                ord_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lead_bid'].items()])
                ord_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lead_ask'].items()])
                self.model_update_bool = False
            # If we are balancing add lift orders as well
            if self.stats.balancing:
                ord_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lift'].items()])
            # just call act, to trigger all product synthetic orders
            try:
                res = self._call_act_orders(timestamp, ord_id_list)
            except Exception as err:
                # self.delete_standing_orders()
    ...
    def custom_on_trade_update(self, trades, timestamp):
        ...
        model_update_bool = False
        for trade in trades:
            if (trade.aggressor_broker_id != "1441") or (trade.initiator_broker_id != "1441"):
                model_update_bool = True
            if trade.tags.get("portfolio_key") == self.strategy_id and trade.trade_id not in self.trade_id_list:
                if trade.tags.get("stats") in ['MarketMakingLimitAsk', 'MarketMakingLimitBid']:
                    # Only lifts execute
                    try:
                        # trd_id_list.extend(self.opp_slot_dict[trade.tags.get("strategy_slot")])
                        # All synthetic orders are called
                        trd_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lift'].items()])
                        trd_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lead_bid'].items()])
                        trd_id_list.extend([(k, v[1]) for k, v in self.synth_slot_dict['lead_ask'].items()])
                    except KeyError:
                        pass
                ...
                self.debug_log("Trade NOT recorded. Tag: {}, id: {}".format(trade.tags.get("portfolio_key"),
                                                                            trade.trade_id))
        # Check if update model only if we have brokers trade others are in public
        if model_update_bool:
            # Update prices for model
            self.stats.update_strategy_prices_all(self.all_delivery_areas, self.exchange.products, None)
            # Update model & mtm
            update_bool = self.stats.update_model_prices(self.model)
            # If we have not yet updated quotes from previous iteration then keep True
            if not self.model_update_bool:
                self.model_update_bool = update_bool
            # Log data
            self.log_model()
        # Check balancing
        self.stats.check_balancing()
        ...
        if run_bool:
            ...
            self.quote()
            try:
                res = self._call_act_orders(timestamp, trd_id_list)
            except Exception as err:
                # self.delete_standing_orders()
            ...
            self.debug_log(
                "custom_on_trade_update: No act resulted from trade. Tag: {}, id: {}".format(
                    trades[0].tags.get("portfolio_key"), trades[0].trade_id))
            res = {}
        return res
    ...
    def _call_act_orders(self, timestamp, trd_id_list):
        products = []
        for inst_id in set(self.inst_id_list):
            products.extend(self.exchange.products.get_active_products(inst_id))
        products_ord, original_mapping_dict = self._order_synth_list(products, trd_id_list)
    ...
    def _order_synth_list(self, products, trd_id_list):
        if trd_id_list:
    ...
    def create_lift_synthetic_order_payload_register(self, so):
        ...
        self.synth_slot_dict['lift'][so.other_product_id] = (so.other_product_id, slot_name)
        if InstrumentKey(so.other_instrument_id, so.other_product_id) == self.leg1_instrument_key:
            broker_list = self.broker1_list
        else:
            broker_list = self.broker2_list
        return {
    ...
    def create_bid_synthetic_order_payload_register(self, instrument_key, other_instrument_key, margin, size_margin,
        ...
        ctrs_name = "LIFT_" + other_instrument_key.instrument_id
        self.opp_slot_dict[slot_name] = [(other_instrument_key.product_id, ctrs_name)]
        self.opp_lead_dict['buy'][other_instrument_key.product_id] = (instrument_key.product_id, slot_name)
        if self.stats.quote_bool_dict[instrument_key.key]:
            self.synth_slot_dict['lead_bid'][instrument_key.product_id] = (instrument_key.product_id, slot_name)
        if instrument_key == self.leg1_instrument_key:
            broker_list = self.broker1_list
        else:
            broker_list = self.broker2_list
        return {
            ...
            "configuration": {
                ...
                "strategy_stats_dict": self.stats,
                "venue_change_bool": self.venue_change_bool,
    ...
    def create_ask_synthetic_order_payload_register(self, instrument_key, other_instrument_key, margin, size_margin,
        ...
        ctrs_name = "LIFT_" + other_instrument_key.instrument_id
        self.opp_slot_dict[slot_name] = [(other_instrument_key.product_id, ctrs_name)]
        self.opp_lead_dict['sell'][other_instrument_key.product_id] = (instrument_key.product_id, slot_name)
        if self.stats.quote_bool_dict[instrument_key.key]:
            self.synth_slot_dict['lead_ask'][instrument_key.product_id] = (instrument_key.product_id, slot_name)
        if instrument_key == self.leg1_instrument_key:
            broker_list = self.broker1_list
        else:
            broker_list = self.broker2_list
