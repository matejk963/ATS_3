...
class MarketMakingOrderBid(SYB.SyntheticOrderBase):
    ...
    def act(self, localview, additional_views, timestamp):
                ...
                if avg_clip_other is not None:
                    ...
                    if self.venue_change_bool:
                        if abs(avg_clip_other - other_sell) < self.tick_size:
                            # Same price as first venue - change venue
                            self.broker_id = self.broker_list[1]
                            if self.verbose:
                                log.debug(
                                    "[{}] MarketMakeBid, venue change to {} for ASK price".format(
                ...
                if avg_clip_other is not None:
                    ...
                    if self.venue_change_bool:
                        if abs(avg_clip_other - other_buy) < self.tick_size:
                            ...
                            if self.verbose:
                                log.debug(
                                    "[{}] MarketMakeBid, venue change to {} for BID price".format(
                                        self.strategy_id, self.broker_id)
                                )
            # Price check
        ...
        if self.check_place_order(our_price) and strategy_stats.quote_bool(local_instrument_key.key, 'bid'):
            return self.create_slot(direction, local_quantity, our_price, info="MarketMakingLimitBid")
        elif self.check_place_order(our_price) and strategy_stats.hold_bool(local_instrument_key.key, 'bid'):
            num_clips = abs(strategy_stats.net_clip_position[local_instrument_key.key])
            if num_clips < 1:
                quantity = round((1 - num_clips) * local_clip, 0)
            else:
                quantity = 0
            return self.create_slot(direction, quantity, our_price, info="MarketMakingLimitBid")
        else:
            return self.remove()
