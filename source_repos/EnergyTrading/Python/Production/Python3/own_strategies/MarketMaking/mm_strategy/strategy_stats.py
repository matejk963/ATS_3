...
class StrategyStats(AdditionalTypeABC):
    ...
    param_dict = {}
    price_coeff = []
    bool_dict = {'balancing': False, 'manage_pos': False,
    ...
    def w(self, value):
        if isinstance(value, float):
            ...
            raise ValueError(
                "StrategyStats weight for equilibrium model must be float, instead was: %s." % value)
    ...
    def w1(self, value):
        if isinstance(value, float):
            ...
            raise ValueError(
                "StrategyStats weight for margin vs std must be float, instead was: %s." % value)
    ...
    def w_std(self, value):
        if isinstance(value, float):
            ...
            raise ValueError(
                "StrategyStats weight for std must be float, instead was: %s." % value)
    ...
    def eql_value(self, value):
        if isinstance(value, float):
            ...
            raise ValueError(
                "StrategyStats value for equilibrium model must be float, instead was: %s." % value)
    ...
    def balancing(self, value):
        if isinstance(value, bool):
            ...
            raise ValueError(
                "StrategyStats balancing must be boolean, instead was: %s." % value)
    ...
    def manage_pos(self, value):
        if isinstance(value, bool):
            ...
            raise ValueError(
                "StrategyStats manage_pos must be boolean, instead was: %s." % value)
    ...
    def hold_bool(self, inst_key, side):
        pos = self.net_clip_position[inst_key]
        if side == 'bid' and inst_key == self.leg1_instkey and pos < 0:
            return False
        elif side == 'bid' and inst_key == self.leg2_instkey and pos > 0:
            return False
        elif side == 'ask' and inst_key == self.leg1_instkey and pos > 0:
            return False
        elif side == 'ask' and inst_key == self.leg2_instkey and pos < 0:
            return False
        else:
    ...
    def get_instrument_model_price(self, instrument_key):
        if any([x is None for x in list(self.price_leg_dict[instrument_key].values())]):
            ...
            price = .5 * (self.price_leg_dict[instrument_key]
                          ['buy'] + self.price_leg_dict[instrument_key]['sell'])
        return price
    ...
    def update_instruments(self, instrument_list, leg1_clip, leg2_clip, eql_w, eql_price, w1, w_std, quote_list,
        ...
        # PriceVol dictionary
        self.price_leg_dict = {k: {s: None for s in [
            'buy', 'sell']} for k in instrument_list[:2]}
        ...
        self.leg_clip_dict = {k: clp for k, clp in zip(
            instrument_list[:2], leg_clip_list)}
        self.param_dict = {
            'eql_weight': eql_w, 'eql_value': eql_price, 'mrg_weight': w1, 'std_weight': w_std}
        ...
        self.quote_bool_dict = {k: b for k, b in zip(
            instrument_list[:2] + ['buy', 'sell'], quote_list)}
    ...
    def manage_trade_position(self, instrument_key, price, quantity, side, timestamp):
        ...
        if bid_clip_spread > 0:
            ...
            trade_leg_dict[self.leg1_instkey]['buy'] -= bid_clip_spread * \
                self.leg1_clip
            trade_leg_dict[self.leg2_instkey]['sell'] -= bid_clip_spread * \
                self.leg2_clip
        ...
        if ask_clip_spread > 0:
            ...
            trade_leg_dict[self.leg1_instkey]['sell'] -= ask_clip_spread * \
                self.leg1_clip
            trade_leg_dict[self.leg2_instkey]['buy'] -= ask_clip_spread * \
                self.leg2_clip
            ...
            trade_spread_dict['sell'] += PriceVol(
                spread_price, ask_clip_spread)
        ...
        leg2_net_clip_position = min(trade_leg_dict[self.leg2_instkey]['buy'] // self.leg2_clip,
                                     trade_leg_dict[self.leg2_instkey]['sell'] // self.leg2_clip)
        net_clip_position_spread = min(
            trade_spread_dict['buy'].quantity, trade_spread_dict['sell'].quantity)
        log.debug(
        ...
        if leg1_net_clip_position > 0:
            ...
            self.pnl += self.pnl_push(
                trade_leg_dict[self.leg1_instkey], leg1_net_clip_position)
        if leg2_net_clip_position > 0:
            ...
            self.pnl += self.pnl_push(
                trade_leg_dict[self.leg2_instkey], leg2_net_clip_position)
        if net_clip_position_spread > 0:
            ...
            self.pnl += self.pnl_push(trade_spread_dict,
                                      net_clip_position_spread)
        # Write it back to dictionaries
        self.trade_leg_dict.update({k: {s: v.to_dict() for s, v in list(
            d.items())} for k, d in list(trade_leg_dict.items())})
        self.trade_spread_dict.update(
            {s: v.to_dict() for s, v in list(trade_spread_dict.items())})
    ...
    def update_strategy_prices(self, instrument_key, direction, products, broker_id):
        additional_views = FV.ViewFactory(
            products=products, strategy_id=self.strategy_id)
        market_view = additional_views.get_view_for_instrument(
            instrument_key.instrument_id,
            instrument_key.product_id
        )
        update_bool = False
        price = get_avg_market_price_depth(
            direction, market_view, 0, broker_id)
        log.debug(
        ...
        if (self.ref_instkey is not None) and (instrument_key.key == self.ref_instkey):
            try:
                price_diff = abs(self.price_ref_dict[direction] - price)
            except TypeError:
                price_diff = 2 * price_tol
    ...
    def update_strategy_prices_all(self, instkey_list, products, broker_id):
        direction_list = [COMMON.Direction.buy, COMMON.Direction.sell]
        additional_views = FV.ViewFactory(
            products=products, strategy_id=self.strategy_id)
        for instkey in instkey_list:
    ...
    def update_model_prices(self, model_stats):
        update_bool = False
        model_price = model_stats.update_strategy_prices(
            self.bid_spread, self.ask_spread, self.mid_spread)
        # Model update
        try:
            model_diff = abs(self.model_dict['spread'] - model_price)
        except TypeError:
            model_diff = 2 * price_tol
        if model_diff > price_tol:
            update_bool = True
    ...
    def init_strategy_prices(self, products, broker_id, model_stats):
        ...
        for instkey in self.instrument_list:
            instrument_key = InstrumentKey().from_instrument_key(instkey)
            for direction in direction_list:
                self.update_strategy_prices(
                    instrument_key, direction, products, broker_id)
    ...
    def print_spread_positions(self, trade_leg_dict, trade_spread_dict):
        ...
        log.debug(
            "[{}] StrategyStats Positions [BID//ASK] on Leg1: [{}] {}//{}, Leg2: [{}] {}//{}".format(
                self.strategy_id, self.leg1_instkey, trade_l1['buy'].to_string(
                ), trade_l1['sell'].to_string(),
...
def get_avg_market_price_depth_old(direction, market_view, depth, broker_id):
    ...
    if depth == 0:
        ...
        price_levels_list = market_view._summarize_price_level(
            direction, broker_id, only_tradable=only_tradable)
