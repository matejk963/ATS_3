...
class MarketMakingOrderAsk(SYB.SyntheticOrderBase):
    ...
    venue_change_bool = SYNCONF.SyntheticOrderConfigField(
        caption="venue_change_bool",
        expected_type=bool,
        description="Strategy cross venues making boolean",
    )
    ...
    def _price_check(self, direction, our_price, price_other, c_our, c_oth, exist_price, local_buy, local_sell,
                     strategy_stats):
        price = None
        spread_direction = COMMON.Direction.sell
        if our_price is None:
            ...
            if direction == COMMON.Direction.buy:
                if (local_buy is not None) and (local_buy - our_price > self.market_depth):
                    ...
                    log.debug(
                        "[{}] MarketMakeAsk, proposed price too far from market BID {} -- {}".format(
                            self.strategy_id, our_price, local_buy)
                    )
                elif (local_sell is not None) and (our_price >= local_sell):
                    # Inverse bid ask spread check
                    log.debug(
                        "[{}] MarketMakeAsk, proposed price create inverse bo spread BID {} -- {}//{}".format(
                ...
                if (local_sell is not None) and (our_price - local_sell > self.market_depth):
                    ...
                    if add_action < -self.stop_loss:
                        log.debug(
                            "[{}] MarketMakeAsk, proposed price would end up in stop loss {} -- {}".format(
                                self.strategy_id, our_price, add_action)
                        )
                    else:
                        price = our_price
    ...
    def act(self, localview, additional_views, timestamp):
        # Always initiate with the first broker in the list
        self.broker_id = self.broker_list[0]
        # Get the instrument key for the local view
        ...
        log.debug(
            "[{}] MarketMakeAsk, BROKER:{},  [Public BUY/SELL]: Local: {}[{}]: {}//{} --- Other: {}[{}]: {}//{}".format(
        ...
        if (spread_model_price is not None) and (margin is not None):
            if direction == COMMON.Direction.sell:
                ...
                if avg_clip_other is not None:
                    ...
                    if self.venue_change_bool:
                        if abs(avg_clip_other - other_sell) < self.tick_size:
                            self.broker_id = self.broker_list[1]
                            if self.verbose:
                                log.debug(
                                    "[{}] MarketMakeAsk, venue change to {} for ASK price".format(
                                        self.strategy_id, self.broker_id)
                                )
            else:
                ...
                if avg_clip_other is not None:
                    our_price = (c_oth * avg_clip_other - (spread_model_price + (margin + self.size_margin))) / c_our
                    if self.venue_change_bool:
                        if abs(avg_clip_other - other_buy) < self.tick_size:
                            self.broker_id = self.broker_list[1]
                            if self.verbose:
                                log.debug(
                                    "[{}] MarketMakeAsk, venue change to {} for BID price".format(
                                        self.strategy_id, self.broker_id)
                                )
            # Price check
        ...
        log.debug(
            "[{}] MarketMakeAsk order placement: our_price: {} / balancing: {}".format(
                self.strategy_id, our_price, strategy_stats.balancing)
        )

        if self.check_place_order(our_price) and strategy_stats.quote_bool(local_instrument_key.key, 'ask'):
            return self.create_slot(direction, local_quantity, our_price, info="MarketMakingLimitAsk")
        elif self.check_place_order(our_price) and strategy_stats.hold_bool(local_instrument_key.key, 'ask'):
            num_clips = abs(strategy_stats.net_clip_position[local_instrument_key.key])
            if num_clips < 1:
                quantity = round((1 - num_clips) * local_clip, 0)
            else:
                quantity = 0
            return self.create_slot(direction, quantity, our_price, info="MarketMakingLimitAsk")
        else:
            return self.remove()

    def check_place_order(self, our_price):
        return (our_price is not None) and not self.reset_bool
