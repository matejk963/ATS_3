...
def create_feed_1(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        ...
        (GAS_YEARS, TTF_ICE, GAS_YR22_ITEM_ID, BROKER_ID_2, 50.1, COMMON.Direction.sell),
        # Power: item id 19 => 2022 (JAN22-JAN23), # item id 20 => 2023
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID_2, 50, COMMON.Direction.buy),
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID_2, 52.1, COMMON.Direction.sell),
        # Mkt3:
...
def create_feed_2(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (GAS_YEARS, TTF_ICE, GAS_YR22_ITEM_ID, BROKER_ID, 49.9, COMMON.Direction.buy),
        (GAS_YEARS, TTF_ICE, GAS_YR22_ITEM_ID, BROKER_ID, 50.2, COMMON.Direction.sell),
        # Power: item id 19 => 2022 (JAN22-JAN23), # item id 20 => 2023
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 49.9, COMMON.Direction.buy),
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 52.2, COMMON.Direction.sell),
    ]:
        feed.append(
            create_add_order(
                timestamp=timestamp,
...
def create_trade_feed_pt(timestamp):
    feed = []
    counter = 1
    QUANTITY = 2
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        # Trade
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 50., COMMON.Direction.sell)
    ]:
        feed.append(
            create_add_trade(
...
def create_trade_feed_1(timestamp):
    feed = []
    counter = 1
    QUANTITY = 2
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        # Trade
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 50.15, COMMON.Direction.sell)
    ]:
        feed.append(
            create_add_trade(
                timestamp=timestamp + counter,
...
def create_trade_feed_2(timestamp):
    feed = []
    counter = 1
    QUANTITY = 1
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        # Trade
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 51.85, COMMON.Direction.buy),
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 51.85, COMMON.Direction.buy),
        (POWER_YEARS, ITALY_BASELOAD, POWER_2023_ITEM_ID, BROKER_ID, 51.85, COMMON.Direction.buy),
    ]:
        feed.append(
...
class SynthStratConfigTest(unittest.TestCase):
    def setUp(self):
        ...
        self.config = StrategyConfiguration(
            ...
            leg2_clip=3,
            price_coeff=[1,1],
            broker1_ids=[BROKER_ID, BROKER_ID_2],
            broker2_ids=[BROKER_ID, BROKER_ID_2],
            fix_margin=0.1,
            max_margin=0.3,
            preferred_clips=1,
            max_clips=1,
    ...
    def test(self):
        ...
        feed = create_feed_1(self.first_transfer + 5)
        feed.extend(create_trade_feed_pt(self.first_transfer + 10))
        # feed.extend(create_trade_feed_1(self.first_transfer + 75))
        feed.extend(create_feed_2(self.first_transfer + 105))
        # feed.extend(create_trade_feed_2(self.first_transfer + 150))
