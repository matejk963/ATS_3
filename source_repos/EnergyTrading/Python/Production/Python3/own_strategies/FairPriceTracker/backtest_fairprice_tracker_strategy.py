...
import os.path
import unittest
from fairprice_tracker.test_wrap import *
from fairprice_tracker.test_global_vars import global_state
import time
...
from fairprice_tracker.constants import BROKER_ID_1, BROKER_ID_2, INST_ID_1, INST_ID_2, PRODUCT_ID_1, POWER_MONTHS, \
    POWER_ITEM_ID_1

PACKAGE_NAME = "fairprice_tracker"
...
class StrategyTest():
    def setUp(self):
        ...
        self.config = StrategyConfiguration(
            ...
            caption="Sparsity benchmark strategy",
            package_name="fairprice_tracker",
...
def main():
    ...
    sorted_list = sorted(global_state.LOG_LIST, key=lambda x: x['call_order'])

    for x in sorted_list:
        string_entry = x['func'] + ", " + str(x['call_order'])
        if 'act_close' in x['func']:
            string_entry += " " + x['args'][-2]
            string_entry += " " + str(x['args'][-4])
        if 'create_slot' in x['func']:
            string_entry += " " + str(x['args'][-3])
            string_entry += " " + str(x['args'][-2])
        print(string_entry)
