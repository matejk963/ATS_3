...
class CustomStrategy(SYSB.SyntheticOrderStrategyBase):
    ...
    def on_strategy_configuration_update(self, strategy_json):
        ...
        self.instrument_map = strategy_json["instrument_map"]
        self.fair_price_buffer_size = strategy_json["fair_price_buffer_size"]
        ...
        if not self.initialized:
            ...
            self.stats.fair_price_bfr_init(self.instrument_map.values(), self.fair_price_buffer_size)
    ...
    def custom_on_public_trade_update(self, trades, timestamp):
        for trade in trades:
            if not (trade.initiator_broker_id == self.broker_list[0] or trade.aggressor_broker_id == self.broker_list[0]):
                ...
                return {}
            instrument = trade.buy_delivery_area + '_' + trade.product.product_id
            if instrument not in self.instrument_map.keys():
                self.info_log(f"PASS - not an observed instrument")
                return {}
            ...
            self.stats.tr_buffer_push(trade, instrument, self.instrument_map)
