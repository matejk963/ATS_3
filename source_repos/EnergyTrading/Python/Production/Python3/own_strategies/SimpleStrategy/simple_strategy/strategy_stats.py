...
log = logging.getLogger("simple_strategy.strategy_statistics")
...
class StrategyStats(AdditionalTypeABC):
    ...
    def update_strategy_prices(self, instrument_key, direction, products, broker_id):
        ...
        self.price_leg_dict[instrument_key.key][direction] = get_avg_market_price_depth(direction, market_view,
                                                                                        0, broker_id)
        # Print best prices for all broker ids
        log.debug("[{}] direction: {}, price_dict: {}".format(
            self.strategy_id, direction, get_front_prices(direction, market_view)))
...
def get_front_prices(direction, market_view):
    if direction == COMMON.Direction.buy:
        max_bool = True
    else:
        max_bool = False
    filter_orders = market_view.create_order_filter(direction, True, False)
    price_dict = {}
    for market_area in market_view._market_areas:
        orders = market_view._product.orders.get(market_area, filter_orders, broker_id=None,
                                                 only_tradable=True, only_active=True)
        for order in orders:
            if order.broker_id in price_dict.keys():
                if max_bool:
                    price_dict[order.broker_id] = max(price_dict[order.broker_id], order.price)
                else:
                    price_dict[order.broker_id] = min(price_dict[order.broker_id], order.price)
            else:
                price_dict[order.broker_id] = order.price
    return price_dict
