...
from . import so_bac_initial
from . import so_bac_closing

# from constants import BROKER_ID_1, BROKER_ID_2, INST_ID_1, INST_ID_2, PRODUCT_ID


log = logging.getLogger("autotrader.baconv_strategy")
...
class CustomStrategy(SYSB.SyntheticOrderStrategyBase):
    ...
    def on_strategy_configuration_update(self, strategy_json):
        ...
        self.product_id = strategy_json["product_id"]
        ...
        product_id_list = [strategy_json["product_id"]]
        ...
        self.ba_conv_large_threshold = strategy_json["ba_conv_large_threshold"]
        self.ba_conv_small_threshold = strategy_json["ba_conv_small_threshold"]
        self.ba_conv_volatility_threshold = strategy_json["ba_conv_volatility_threshold"]
        self.ba_conv_ba_threshold = strategy_json["ba_conv_ba_threshold"]
    ...
    def custom_on_order_book_update(self, orders, timestamp):
        ...
        #self.stats.increment_bid_ask_list(InstrumentKey.from_instrument_key(self.instrument_keys[0]), self.exchange.products)

        if (self.stats.in_position() and any([instrument_key.key==self.instrument_keys[1] for instrument_key, direction in tuple_list])):
                # (not self.stats.in_position())and self.stats.check_for_new_lead_trade()):
    ...
    def custom_on_public_trade_update(self, trades, timestamp):
        ...
        for trade in trades:
            ...
            if ('OwnTrade' not in str(type(trade))) \
            and trade.aggressor_broker_id=='1441' \
            and trade.initiator_broker_id=='1441':
    ...
    def create_bac_initial_synthetic_order_payload_register(self, instrument_key):
    ...
    def create_bac_closing_synthetic_order_payload_register(self, so):
        return {
            "operation": COMMON.SyntheticOrderOperations.register,
    ...
    def _simple_init_order(self):
        for key in self.instrument_keys:
            ...
            init_orders = [so for so in
                                    ...
                                    isinstance(so, so_bac_initial.InitialOrder)]
            if len(init_orders) == 1:
                ...
                print(("Creating Initial SO {}[{}]".format(
                    instrument_key.instrument_id, instrument_key.product_id)))
                payload = self.create_bac_initial_synthetic_order_payload_register(instrument_key)
    ...
    def _update_closing_orders(self):
        for prod, sos in list(self.product_synthetic_order_mapping.items()):
            for identifier, so in list(sos.items()):
                if isinstance(so, so_bac_closing.ClosingOrder):
                ...
                elif isinstance(so, so_bac_initial.InitialOrder):
                    if any([(closing_so.slot_name == "CLOSING_" + so.slot_name
                             and closing_so.market_area == so.instrument_id)
                            for closing_so in list(self.product_synthetic_order_mapping[so.product_id].values())]):
                        # if we already have a CLOSING for this, then do nothing
                        pass
                    else:
                        self.debug_log("Creating CLOSING SO {} [{}]".format(so.identifier, prod))
                        print("Creating CLOSING SO {} [{}]".format(so.identifier, prod))
                        payload = self.create_bac_closing_synthetic_order_payload_register(so)
    ...
    def check_stats_dict_all(self):
        ...
        bid_ask_list_id = id(self.stats.bid_ask_list)
        so_bid_ask_list_id_list = [id(x.strategy_stats_dict['bid_ask_list']) for tr in
                           self.product_synthetic_order_mapping.values() for x in tr.values()]
        return all([x == slot_dict_id for x in so_slot_dict_id_list]) and all([x == bid_ask_list_id for x in so_bid_ask_list_id_list])

    @classmethod
    def get_available_synthetic_order_types(cls):
        return {
            "InitialOrder": so_bac_initial.InitialOrder,
            "ClosingOrder": so_bac_closing.ClosingOrder
        }
