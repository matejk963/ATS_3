...
log = logging.getLogger("baconv_strategy.predictors")
...
def calculate_rolling_macd_fast(bid_ask_list, side, short_window=7, long_window=16):
    """
    Computes MACD and volatility from a list of bid-ask price updates.

    :param bid_ask_list: List of tuples (bid_price, ask_price, timestamp), with the first being the most recent.
    :param side: 'bid' or 'ask' to determine which price to use.
    :param short_window: Short EMA window size.
    :param long_window: Long EMA window size.
    :return: Tuple (MACD, Volatility)
    """
    ...
    # Extract the chosen price while filtering out None values
    prices = [entry[0] if side == "bid" else entry[1] for entry in bid_ask_list if
              entry[0] is not None and entry[1] is not None]

    if len(prices) < long_window:
        return None, None  # Not enough data to compute MACD
    ...
    def ema(values, window):
        ...
        ema_values.append(values[0])  # Seed EMA with first value
        for price in values[1:(long_window-1) ]:
    ...
    # Compute MACD
    macd = round(short_ema[-1] - long_ema[-1],4)

    # Compute volatility
    volatility = round(max(prices[-long_window:]) - min(prices[-long_window:]),4)
