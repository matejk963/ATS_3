...
class CustomStrategy(STRAT.Strategy):
    ...
    def custom_act_for_product(self, log_data, product, timestamp, *args, **kwargs):
        ...
        if current_best_buy is not None:
            slot = STRAT.PositionSlot("front", COMMON.Direction.sell, 0.1, current_best_buy, info="placement_info")
            slot_resp = self.place_slots(
                log_data, product, timestamp, self.delivery_area_id, [slot]
            )
            ...
            slot = STRAT.PositionSlot("front", COMMON.Direction.sell, pos, price, info="placement_info")
            slot_resp = self.place_slots(
                log_data, product, timestamp, self.delivery_area_id, [slot]
            )
