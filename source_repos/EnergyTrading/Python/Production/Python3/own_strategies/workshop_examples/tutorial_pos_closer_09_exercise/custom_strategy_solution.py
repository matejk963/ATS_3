...
class CustomStrategy(STRAT.Strategy):
    ...
    def custom_act_for_product(self, log_data, product, timestamp, *args, **kwargs):
        ...
        open_position = round(short_target - long_target, 2)

        traded_vals = [
            round(net_traded[ts], 2) for ts in range(product.delivery_start, product.delivery_end, COMMON.QUARTER)
        ]
        if open_position > 0:
            # example: max traded: [-10, 0, 10, 5] => 10, open: 5 ->  -5 => sell 5
            common_traded = round(max(traded_vals), 2)
        else:
            # example: min traded: [-10, 0, 10, 5] => -10, open: -15 ->  -5 => sell 5 more
            common_traded = round(min(traded_vals), 2)

        rest = round(open_position - min(traded_vals), 2)
        ...
        max_slot_size = 0.5
        step_size = 0.1
        dumpmode = product.delivery_start - timestamp <= 1.5 * COMMON.HOUR

        if open_position > 0:
            slot.direction = COMMON.Direction.buy
            slot.quantity = min(rest, max_slot_size)
            if dumpmode:
                mode = "dumpmode"
                if best_public_sell_price is not None:
                    slot.price = best_public_sell_price + step_size
                else:
                    slot.price = self.strategy_limit_maximum_purchase_price.get(product_interval)
            else:
                mode = "normal"
            ...
            slot.quantity = min(abs(rest), max_slot_size)
            if dumpmode:
                mode = "dumpmode"
                if best_public_buy_price is not None:
                    slot.price = best_public_buy_price - step_size
                else:
                    slot.price = self.strategy_limit_minimum_sales_price.get(product_interval)
            else:
                mode = "normal"
                if best_public_sell_price is not None:
                    slot.price = best_public_sell_price - step_size
        ...
        slot.info = "mode: {}||trd: {:.1f}({})||net_target_pos: {:.1f}||rest: {:.1f}||fronts: {}//{}".format(
            mode, float_or_none(common_traded), [float_or_none(val) for val in traded_vals],
            float_or_none(open_position), float_or_none(rest),
            float_or_none(best_public_buy_price), float_or_none(best_public_sell_price)
        )
    ...
    def custom_on_trade_update(self, trades, timestamp):
        self.act(timestamp, set(t.product for t in trades))
