...
class TestPosCloser(SUTB.TestWithEpexProducts):
    ...
    def test(self):
        ...
        self.update_ts({
            COMMON.StrategyJsonKey.TS.pos_sell: 12.0,
            COMMON.StrategyJsonKey.TS.pos_buy: 0.0,
            COMMON.StrategyJsonKey.TS.price_buy: 10.0,
            COMMON.StrategyJsonKey.TS.price_sell: 50.0,
        })
        ...
        ######################################
        # call strategy act with enough time #
        ######################################
        # strat on custom act
        ...
        self.assertEqual(placed_slot.price_range_hi, 50.4)
        self.assertEqual(placed_slot.price_range_lo, 49.4)
        self.assertEqual(placed_slot.price, 49.9)
        self.assertEqual(placed_slot.quantity, 0.5)
        self.assertEqual(placed_slot.slot_type, "front")
        self.assertEqual(placed_slot.direction, "sell")
        self.assertEqual(placed_slot.info,
                         "mode: normal||trd: 10.0([10.0, 10.0, 10.0, 10.0])||net_target_pos: -12.0||rest: -22.0||"
                         "fronts: 30.0//50.0")

        # check 10 to 11 product
        placed_slot = placements[(self._10_11_prod_id, "front")]

        self.assertEqual(placed_slot.price_range_hi, 50.5)
        self.assertEqual(placed_slot.price_range_lo, 49.5)
        self.assertEqual(placed_slot.price, 50)
        self.assertEqual(placed_slot.quantity, 0.5)
        self.assertEqual(placed_slot.slot_type, "front")
        self.assertEqual(placed_slot.direction, "sell")
        self.assertEqual(placed_slot.info,
                         "mode: normal||trd: -5.0([-5.0, -5.0, -5.0, -5.0])||net_target_pos: -12.0||rest: -7.0||"
                         "fronts: None//None")

        ########################################################################
        # call strategy act with less than 1 hour time => should use dump mode #
        ########################################################################
        # strat on custom act
        placements = self.run_strategy_and_get_placed_slots(
            self.first_delivery_start - (45 * COMMON.MINUTE),
            [self.product_09_10, self.product_10_11]
        )
        ...
        # first product is dumped and second product is placed with normal mode

        # check 9 to 10 product
        placed_slot = placements[(self._09_10_prod_id, "front")]

        self.assertEqual(placed_slot.price_range_hi, 30.4)
        self.assertEqual(placed_slot.price_range_lo, 29.4)
        self.assertEqual(placed_slot.price, 29.9)
        self.assertEqual(placed_slot.quantity, 0.5)
        self.assertEqual(placed_slot.slot_type, "front")
        self.assertEqual(placed_slot.direction, "sell")
        self.assertEqual(placed_slot.info,
                         "mode: dumpmode||trd: 10.0([10.0, 10.0, 10.0, 10.0])||net_target_pos: -12.0||rest: -22.0||"
                         "fronts: 30.0//50.0")
        # check 10 to 11 product
        placed_slot = placements[(self._10_11_prod_id, "front")]
