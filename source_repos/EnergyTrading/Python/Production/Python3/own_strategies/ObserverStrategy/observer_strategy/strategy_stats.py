...
log = logging.getLogger("observer_strategy.strategy_statistics")
...
class StrategyStats(AdditionalTypeABC):
    ...
    pnl_dict = {}
    market_data_dict = {}
    lead_pers_dict = {'flag': False, 'time': None, 'price': None, 'volume': None}
    aux_dict = {'ql': None, 'locked_until': 0.0, 'timestamp': None, 'last_traded_price': None,
                'trade_id': None, 'steering_closing': 'auto', 'open_price': None, 'make_profit_margin': None, 'trd_object': None}
    ...
    def update_instruments(self, instrument_list, time_of_init, params):
        ...
        self.aux_dict = {'ql': None, 'locked_until': 0.0, 'timestamp': None, 'last_traded_price': None,
                    'trade_id': None, 'steering_closing': 'auto', 'open_price': None, 'make_profit_margin': None, 'trd_object': None}
        self.param_dict = {'make_profit_margin': params.get('make_profit_margin', 0.25), 'makeagg_ratio': None, 'bid_price': None, 'ask_price': None,
                           'open_price': None, 'burnout_period': 0.0, 'branch_flag': 0,
                           'a_price_sparsity': 0.0, 'b_price_sparsity': 0.0,
                  'position_list': [{'timestamp': None, 'direction': None, 'price': None, 'volume': None} for _ in range(50)]}
        self.lead_pers_dict = {'flag': False, 'time': None, 'price': None, 'volume': None}
        self.position_dict = self.init_position_dict
        self.trade_decision_log = {
            'trade_id': [],
            'reason': []
    ...
    def set_lead_persistance(self, flag, time, price, volume):
        self.lead_pers_dict['flag'] = flag
        self.lead_pers_dict['time'] = time
        self.lead_pers_dict['price'] = price
        self.lead_pers_dict['volume'] = volume
    ...
    def update_trade_info(self, trade, timestamp):
        ...
        self.aux_dict['trd_object'] = trade

    def market_data_update(self, timestamp, trade_id, trd_price, b_price, a_price):
        self.market_data_dict['timestamp'].append(timestamp)
        self.market_data_dict['trade_id'].append(trade_id)
        self.market_data_dict['trd_price'].append(trd_price)
        self.market_data_dict['b_price'].append(b_price)
        self.market_data_dict['a_price'].append(a_price)

    def predictors_update(self, timestamp, trade_id, predictors):
        self.predictor_dict['timestamp'].append(timestamp)
        self.predictor_dict['trade_id'].append(trade_id)
        self.predictor_dict['predictors'].append(predictors)


    def log_trade_decision(self, trade_id, reason):
        """
        Log trade decision with all conditions and outcomes
        
        :param trade_id: ID of the public trade
        :param timestamp: Trade timestamp
        :param decision_data: Dictionary containing all decision conditions and outcomes
        """
        
        self.trade_decision_log['trade_id'].append(trade_id)
        self.trade_decision_log['reason'].append(reason)

        log.info(f"[TRADE_DECISION_LOG] Trade {trade_id}: {reason}")

    def get_recent_trade_decisions(self, count=10):
        """
        Get most recent trade decisions
        
        :param count: Number of recent decisions to return
        :return: List of recent trade decision entries
        """
        return self.trade_decision_log[-count:] if self.trade_decision_log else []
    
    def get_trade_decisions_by_outcome(self, entry_decision=None):
        """
        Filter trade decisions by outcome
        
        :param entry_decision: 'enter', 'no_enter', 'filtered', or None for all
        :return: List of filtered trade decision entries
        """
        if entry_decision is None:
            return self.trade_decision_log
        return [entry for entry in self.trade_decision_log if entry.get('entry_decision') == entry_decision]
    
    def get_comprehensive_trade_analysis(self):
        """
        Get comprehensive analysis of all observed trades with metrics and conditions
        """
        comprehensive_analysis = []
        
        for decision in self.trade_decision_log:
            if 'comprehensive_metrics' in decision:
                comprehensive_analysis.append({
                    'trade_id': decision['trade_id'],
                    'timestamp': decision['timestamp'],
                    'decision_data': decision,
                    'metrics': decision['comprehensive_metrics']
                })
        
        return comprehensive_analysis
    
    def export_comprehensive_metrics_for_steering(self):
        """
        Export comprehensive metrics for steering calls (production use)
        """
        metrics_data = []
        ...
        for decision in self.trade_decision_log:
            if 'comprehensive_metrics' in decision:
                metrics_data.append({
                    ...
                    'comprehensive_metrics': decision['comprehensive_metrics']
                })
        
        return {
            'total_observations': len(metrics_data),
            'trading_conditions_summary': self.get_trading_conditions_summary(),
            'comprehensive_metrics': metrics_data,
            'export_timestamp': time.time()
        }
    
    def get_trading_conditions_summary(self):
        """
        Get summary of trading conditions across all observed trades
        """
        summary = {
        ...
        for decision in self.trade_decision_log:
            if 'comprehensive_metrics' in decision:
                summary['total_trades_observed'] += 1
                metrics = decision['comprehensive_metrics']
                conditions = metrics['trading_conditions']
                
                long_met = conditions['long_possible']
                short_met = conditions['short_possible']
                
                if long_met and short_met:
                    summary['both_conditions_met'] += 1
                elif long_met:
                    summary['long_conditions_met'] += 1
                elif short_met:
                    summary['short_conditions_met'] += 1
                else:
                    summary['no_conditions_met'] += 1
                
                summary['conditions_breakdown'].append({
                    'trade_id': metrics['trade_id'],
                    'long_possible': long_met,
                    'short_possible': short_met,
                    'trade_price': metrics['trade_price'],
                    'bid_price': metrics['order_book_metrics']['bid_price'],
                    'ask_price': metrics['order_book_metrics']['ask_price'],
                })
