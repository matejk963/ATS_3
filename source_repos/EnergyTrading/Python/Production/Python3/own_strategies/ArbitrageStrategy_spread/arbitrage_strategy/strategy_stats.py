...
from .own_tools.trade_artificial_class import ArtTrade
...
class StrategyStats(AdditionalTypeABC):
    ...
    @property
    def spread_instkey(self):
        return self.instrument_list[0] if self.instrument_list else None

    @property
    def leg1_instkey(self):
        return self.instrument_list[1] if self.instrument_list else None

    @property
    def leg2_instkey(self):
        return self.instrument_list[2] if self.instrument_list else None
    ...
    def manage_trade_position(self, instrument_key, price, quantity, side, timestamp):
        ...
        if side == COMMON.Direction.sell:
            ...
            if instrument_key == self.spread_instkey:
        ...
        # Update net position
        bid_spread_position = min(trade_leg_dict[self.leg1_instkey]['buy'].quantity,
                                  trade_leg_dict[self.leg2_instkey]['sell'].quantity)
        ask_spread_position = min(trade_leg_dict[self.leg2_instkey]['buy'].quantity,
                                  trade_leg_dict[self.leg1_instkey]['sell'].quantity)
        # Output artificial spread trade if exists
        art_trade = None
        # Bid spread
        if bid_spread_position > 0:
            # Reduce position on legs from bid side
            trade_leg_dict[self.leg1_instkey]['buy'] -= bid_spread_position
            trade_leg_dict[self.leg2_instkey]['sell'] -= bid_spread_position
            spread_price = trade_leg_dict[self.leg1_instkey]['buy'].spread_sub(
                trade_leg_dict[self.leg2_instkey]['sell'])
            trade_spread_dict['buy'] += PriceVol(
                spread_price, bid_spread_position)
            # Push it to spread instrument
            trade_leg_dict[self.spread_instkey]['buy'] += PriceVol(
                spread_price, bid_spread_position)
            art_trade = ArtTrade(
                direction=COMMON.Direction.buy,
                priceVol_class=PriceVol(spread_price, bid_spread_position),
                execution_time=timestamp
            )
        # Ask spread
        if ask_spread_position > 0:
            # Reduce position on legs from ask side
            trade_leg_dict[self.leg2_instkey]['buy'] -= ask_spread_position
            trade_leg_dict[self.leg1_instkey]['sell'] -= ask_spread_position
            spread_price = trade_leg_dict[self.leg1_instkey]['sell'].spread_sub(
                trade_leg_dict[self.leg2_instkey]['buy'])
            trade_spread_dict['sell'] += PriceVol(
                spread_price, ask_spread_position)
            art_trade = ArtTrade(
                direction=COMMON.Direction.sell,
                priceVol_class=PriceVol(spread_price, ask_spread_position),
                execution_time=timestamp
            )
            # Push it to spread instrument
            trade_leg_dict[self.spread_instkey]['sell'] += PriceVol(
                spread_price, ask_spread_position)
        # Clear closed position
        leg1_net_position = min(trade_leg_dict[self.leg1_instkey]['buy'].quantity,
                                trade_leg_dict[self.leg1_instkey]['sell'].quantity)
        leg2_net_position = min(trade_leg_dict[self.leg2_instkey]['buy'].quantity,
                                trade_leg_dict[self.leg2_instkey]['sell'].quantity)
        net_position_spread = min(
        ...
        if self.verbose:
            log.debug(
                "[{}] StrategyStats Management of position clearing clips: Leg1 // Leg2 // Spread: {} // {} // {}".format(
                    self.strategy_id, leg1_net_position, leg2_net_position, net_position_spread)
            )
        if net_position_spread > 0:
            # Clear spread position
            trade_spread_dict['buy'] -= net_position_spread
            trade_spread_dict['sell'] -= net_position_spread
            # self.pnl += self.pnl_push(trade_spread_dict, net_position_spread)
    ...
    def pnl_push(self, pv_dict, trade_pos):
        return PriceVol.cash_sub(pv_dict['sell'], pv_dict['buy'], trade_pos)

    def pnl_calc_and_push(self):
        """ Calculates and pushes the PNL"""
        pnl_dict_per_instrument = self.pnl_dict_per_instrument
        new_pnl = pnl_dict_per_instrument[self.spread_instkey]
        if new_pnl < self.pnl:
    ...
    def initiate_slot_dict(self, instrument_list, time_of_init):
        self.slot_dict = {}
        slot_name_list = ["Arb_Order_BID_", "Arb_Order_ASK_",
                          "LIFT_Arb_Order_BID_", "LIFT_Arb_Order_ASK_"]
...
def get_price_time_depth(strategy_id, direction, market_view, price_level, depth, lead_broker_id, min_price_level, verbose):
    ...
    # Iterate through the summarized price levels
    time_list = [0, lift_update_time[1]]
    ...
    if verbose:
        log.debug(
            "[{}] get_price_time_depth({},{}) calculated vol: {}, lift_ins/bck_ins: {}//{}. ".format(
                strategy_id, direction, depth, vol, time_list[1], time_list[0])
        )
