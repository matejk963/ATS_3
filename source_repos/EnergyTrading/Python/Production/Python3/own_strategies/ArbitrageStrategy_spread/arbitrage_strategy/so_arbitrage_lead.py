...
class ArbitrageOrder(SYB.SyntheticOrderBase):
    """Base class for arbitrage orders containing common functionality"""
...
class ArbitrageOrderBid(ArbitrageOrder):
    """Bid-specific arbitrage order implementation"""
...
class ArbitrageOrderAsk(ArbitrageOrder):
    """Ask-specific arbitrage order implementation"""
    ...
    def act(self, localview, additional_views, timestamp):
        ...
        if self.check_place_order_ask(abs_net_open_position, strategy_stats):
            own_price, own_brk = self.get_own_order_price(
