...
log = logging.getLogger("autotrader.arbitrage_strategy_spread")
tol = 1e-3
...
class CustomStrategy(SYSB.SyntheticOrderStrategyBase):
    ...
    def instrument_keys(self):
        return [self.instrument_keys_all[0]]

    def params_cons_making_validation(self, strategy_json):
        # name_list = ["cons_making", "cons_making_aux", "cons_margin", "cons_min_level"]
        cons_making_dict = {
            k: strategy_json["cons_making"] for k in self.broker_ids_main}
        cons_making_dict.update({k: strategy_json["cons_making_aux"]
                                for k in self.broker_ids if k not in self.broker_ids_main})
    ...
    def params_flex_margin_validation(self, strategy_json):
        name_list = ["min_margin", "max_margin",
                     "opt_margin", "min_margin_aux"]
        min_margin, max_margin, opt_margin, min_margin_aux = [
            abs(strategy_json[name]) for name in name_list]
        ...
        self.min_margin_dict.update(
            {k: min_margin_aux for k in self.broker_ids if k not in self.broker_ids_main})
    ...
    def on_strategy_configuration_update(self, strategy_json):
        ...
        self.instrument_keys_all = [InstrumentKey(inst_id, prod_id).key
                                    for (inst_id, prod_id) in zip(instrument_id_list, product_id_list)]
        # parameters of strategy
        self.instrument_ids = [instrument_id_list[0]]
        ...
        self.exchange_broker_ids = strategy_json["exchange_broker_ids"]
        ...
        self.maximum_neg_buyback = strategy_json.get(
            COMMON.StrategyJsonKey.maximum_neg_buyback, None) or 4
        ...
        assert self.market_volume_check_flag in [
            0.0, 1.0], 'Parameter market_volume_check_flag must be 1 or 0!'
        assert self.closing_in_profit_flag in [
            0.0, 1.0], 'Parameter closing_in_profit_flag must be 1 or 0!'

        self.stats = StrategyStats(
            None, self.strategy_id, self.caption, self.verbose)
        ...
        if not self.initialized:
            self.opp_slot_dict = {}
            self.prod_slot_dict = {'buy': {}, 'sell': {}}
            self.delete_standing_orders()
            self.stats.update_instruments(
                self.instrument_keys_all, self.time_of_init)
            self.log_stats_dict = {k: [] for k in self.instrument_keys_all}
        ...
        if strategy_json.get(COMMON.StrategyJsonKey.active):
            if self.broker_making:
                self.shuffle_broker_list = [
                    x for x in np.random.permutation(self.broker_ids[1:])]
            self.init_orders = True
            self.fexe_bool = False
            self.log_stats_dict = {k: [] for k in self.instrument_keys_all}
    ...
    def handle_steering_call(self, payload):
        ...
        if "stats" in payload:
            ...
            self.debug_log("[{}] Unknown steering call code: {}".format(
    ...
    def custom_on_order_book_update(self, orders, timestamp):
        ...
        # run_bool = False
        # trade_bool = True
        fexe_dict = {instkey: {s: False for s in [
            'buy', 'sell']} for instkey in self.instrument_keys}
        if self.init_orders:
        ...
        for o in orders:
            instrument_key = InstrumentKey(
                o.delivery_area_id, o.product.product_id)
            if instrument_key.key in self.instrument_keys:
                ...
                if isinstance(o, APITR.OwnOrder) and o._fexe:
                    ...
                    self.debug_log(
                        "DEBUG ORDER BOOK UPDATE: skipping call act due to FULL EXECUTION of order")
                    continue
                ...
                try:
                    ...
                    # ARB OPP SIDE
                    synth_name_lead_opp = self.prod_slot_dict[COMMON.Direction().invert(
                        o.direction)][instrument_key.product_id]
                    synth_name_lift_opp = self.opp_slot_dict[synth_name_lead_opp][1]
                    # Determine open positions
                    net_arb_vol = self.stats.slot_dict_net_position(
                        synth_name_lead, synth_name_lift)
                    net_arb_opp_vol = self.stats.slot_dict_net_position(
                        synth_name_lead_opp, synth_name_lift_opp)
                    ...
                    ord_id_list.extend(
                        [(instrument_key.product_id, sn) for sn in synth_name])
        ...
        if run_bool:
            ...
            try:
                res = self._call_act_orders(
                    timestamp, ord_id_list, trade_bool, True)
            except Exception as err:
                # self.delete_standing_orders()
    ...
    def custom_on_trade_update(self, trades, timestamp):
        ...
        for trade in trades:
            if trade.tags.get("portfolio_key") == self.strategy_id:
                if trade.trade_id not in self.trade_id_list:
                    if trade.tags.get("stats") in ['leadSell', 'leadBuy']:
                        try:
                            trd_id_list.append(
                                self.opp_slot_dict[trade.tags.get("strategy_slot")])
                        except KeyError:
                            pass
        ...
        if not trd_id_list:
            ...
            position_dict = {v: self.stats.slot_dict_net_position(
                k, v[1]) for k, v in self.opp_slot_dict.items()}
        ...
        if run_bool:
            try:
                res = self._call_act_orders(
                    timestamp, trd_id_list, True, False)
                self.fexe_bool = False
            except Exception as err:
                # self.delete_standing_orders()
    ...
    def custom_on_public_trade_update(self, trades, timestamp):
        ...
        if not instrument_tuple_list:
            ...
            self.debug_log(
                "DEBUG on_public_trade_update: skipping call act due to FEXE")
            self.fexe_bool = False
        ...
        if not synth_ord_list:
            ...
            try:
                res = self._call_act_orders(
                    timestamp, synth_ord_list, True, False)
            except Exception as err:
                # self.delete_standing_orders()
    ...
    def create_bid_synthetic_order_payload_register(self, instrument_key, margin):
        ...
        return {
            ...
            "synthetic_order_type": "ArbitrageOrderBid",
            ...
            "configuration": {
                ...
                "cons_min_level": self.cons_min_level,
    ...
    def create_ask_synthetic_order_payload_register(self, instrument_key, margin):
        ...
        return {
            ...
            "product_id": instrument_key.product_id,
            "message_type": "synthetic_order",
            "synthetic_order_type": "ArbitrageOrderAsk",
    ...
    def _simple_order_buy(self, margin):
        for key in self.instrument_keys:
            ...
            market_making_orders = [so for so in
                                    ...
                                    isinstance(so, so_arbitrage_lead.ArbitrageOrderBid)]
            if len(market_making_orders) == 1:
                ...
                if self.reset_bool:
                    payload = {"identifier": so.identifier, "configuration": {
                        "margin": margin, "reset_bool": self.reset_bool}}
                    self.on_synthetic_order_modify(
                        instrument_key.product_id, payload)
                ...
                for so in market_making_orders[1:]:
                    ...
                    payload = {"identifier": so.identifier,
                               "synthetic_order_type": "ArbitrageOrderBid"}
                    self.on_synthetic_order_delete(
                        instrument_key.product_id, payload)
            else:
                # Create buy order
                ...
                print(("Creating ArbitrageBid SO {}[{}]".format(
                    instrument_key.instrument_id, instrument_key.product_id)))
                payload = self.create_bid_synthetic_order_payload_register(
                    instrument_key, margin)
                self.on_synthetic_order_register(
                    instrument_key.product_id, payload)
    ...
    def _simple_order_sell(self, margin):
        for key in self.instrument_keys:
            ...
            market_making_orders = [so for so in
                                    list(self.product_synthetic_order_mapping[instrument_key.product_id].values()) if
                                    isinstance(so, so_arbitrage_lead.ArbitrageOrderAsk)]
            if len(market_making_orders) == 1:
                # Modify order
                so = market_making_orders[0]
                if self.verbose:
                ...
                print(("Creating ArbitrageAsk SO {}[{}]".format(
                    instrument_key.instrument_id, instrument_key.product_id)))
                payload = self.create_ask_synthetic_order_payload_register(
                    instrument_key, margin)
                self.on_synthetic_order_register(
                    instrument_key.product_id, payload)
    ...
    def _update_lift_orders(self):
        for prod, sos in list(self.product_synthetic_order_mapping.items()):
            for identifier, so in list(sos.items()):
                if isinstance(so, so_arbitrage_lift.LiftOrder) and 'BID' in so.slot_name:
                    if any([(lead_so.slot_name.startswith("Arb_Order_BID")
                        ...
                        if self.reset_bool:
                            ...
                            self.debug_log(
                                "Found LEAD for LIFT SO {} [{}]".format(so.identifier, prod))
                    else:
                        self.debug_log(
                            "Removing LIFT SO {} [{}]".format(so.identifier, prod))
                        payload = {"identifier": so.identifier,
                                   "synthetic_order_type": "LiftOrder"}
                ...
                elif isinstance(so, (so_arbitrage_lead.ArbitrageOrderBid, so_arbitrage_lead.ArbitrageOrderAsk)):
                    if any([(lift_so.slot_name == "LIFT_" + so.slot_name
                        ...
                        if self.reset_bool:
                            payload = {"identifier": so.identifier, "configuration": {
                                "reset_bool": self.reset_bool}}
                            self.on_synthetic_order_modify(
                                so.lift_product_id, payload)
                            self.debug_log(
                                "Found LIFT for LEAD SO RESET BOOL modify {} [{}]".format(so.identifier, prod))
                    else:
                        self.debug_log(
                            "Creating LIFT SO {} [{}]".format(so.identifier, prod))
                        payload = self.create_lift_synthetic_order_payload_register(
                            so)
                        self.on_synthetic_order_register(
                            so.lift_product_id, payload)
    ...
    def delete_standing_orders(self):
        for prod, sos in list(self.product_synthetic_order_mapping.items()):
            for identifier, so in list(sos.items()):
                if isinstance(so, so_arbitrage_lead.ArbitrageOrderBid):
                    self.debug_log(
                        "Removing ArbitrageOrderBid SO {} [{}]".format(so.identifier, prod))
                    payload = {"identifier": so.identifier,
                               "synthetic_order_type": "ArbitrageOrderBid"}
                    self.on_synthetic_order_delete(prod, payload)
                elif isinstance(so, so_arbitrage_lead.ArbitrageOrderAsk):
                    self.debug_log(
                        "Removing ArbitrageOrderAsk SO {} [{}]".format(so.identifier, prod))
                    payload = {"identifier": so.identifier,
                               "synthetic_order_type": "ArbitrageOrderAsk"}
                    self.on_synthetic_order_delete(prod, payload)
    ...
    def _call_act_orders(self, timestamp, trd_id_list=[], trade_bool=False, ordup_bool=True):
        ...
        for inst_id in set(self.instrument_ids):
            products.extend(
                self.exchange.products.get_active_products(inst_id))
        products_ord, original_mapping_dict = self._order_synth_list(
            products, trd_id_list, trade_bool)
        res = self.act(timestamp, products_ord)
        if bool(original_mapping_dict):
            # Reorder back to original mapping
            print_dict = {
                p: list(v.keys()) for p, v in self.product_synthetic_order_mapping.items()}
    ...
    def _order_synth_list(self, products, trd_id_list, trade_bool):
        if trd_id_list:
            ...
            products_ord = []
            prod_mapping_dict = {k: {}
                                 for k in self.product_synthetic_order_mapping.keys()}
            # Reorder products
            for t_id in trd_id_list:
                if not prod_mapping_dict[t_id[0]]:
                    i_d = product_list.index(t_id[0])
                    products_ord.append(products[i_d])
                prod_mapping_dict[t_id[0]][t_id[1]
                                           ] = self.product_synthetic_order_mapping[t_id[0]][t_id[1]]
            # Reorder synthetic orders
            original_mapping_dict = self._reorder_mapping_dict(
                prod_mapping_dict, trade_bool)
        else:
    ...
    def _reorder_mapping_dict(self, new_mapping_dict, trade_bool):
        ...
        for p, s in new_mapping_dict.items():
            original_mapping_dict[p] = {
                k: v for k, v in self.product_synthetic_order_mapping[p].items()}
            if trade_bool:
                ...
                ordered_dict = {**s, **{k: v for k, v in self.product_synthetic_order_mapping[p].items() if
                                        k not in s.keys()}}
            self._product_synthetic_order_mapping[p] = {
                k: v for k, v in ordered_dict.items()}
        return original_mapping_dict

    def __update_status_balancing(self):
        instrument_key_list = [InstrumentKey().from_instrument_key(key)
                               for key in self.instrument_keys]
        ...
        for inst_key in instrument_key_list:
            ...
            if any([status != COMMON.SyntheticOrderStates.passive for status in list(lift_orders_bid_dict.values())]):
                ...
                self.debug_log("self.stats.balancing_bid for " +
                               str(inst_key.key)+" set to True")
            else:
                self.stats.balancing_dict[inst_key.key]['BID'] = False
                self.debug_log("self.stats.balancing_bid for " +
                               str(inst_key.key)+" set to False")
            ...
            if any([status != COMMON.SyntheticOrderStates.passive for status in list(lift_orders_ask_dict.values())]):
                ...
                self.debug_log("self.stats.balancing_ask for " +
                               str(inst_key.key)+" set to True")
            else:
    ...
    def _process_single_trade(self, trade, timestamp):
        ...
        try:
            if trade.aggressor_broker_id in self.exchange_broker_ids:
                # Exchange venue - only legs
                if instrument_key in [self.stats.leg1_instkey, self.stats.leg2_instkey]:
                    # Ignore trade on spread instrument key only do legs
                    art_trade = self.stats.manage_trade_position(
                        instrument_key, trade.price, trade.quantity, direction, timestamp)
                    if art_trade is not None:
                        # If we have a trade on legs then increment slot dict
                        art_trade.slot_name = trade.tags.get('strategy_slot')
                        self.stats.increment_slot_dict_trade(art_trade)
            else:
                # Broker venue - only spreads
                if instrument_key == self.stats.spread_instkey:
                    # Ignore trade on legs instrument key only do spread
                    self.stats.manage_trade_position(
                        instrument_key, trade.price, trade.quantity, direction, timestamp)
    ...
    def check_stats_dict_all(self):
        ...
        so_stat_id_list = [id(x.strategy_stats_dict['slot_dict'])
                           for tr in self.product_synthetic_order_mapping.values() for x in tr.values()]
        return all([x == stat_id for x in so_stat_id_list])

    @classmethod
    def get_available_synthetic_order_types(cls):
        return {
            "ArbitrageOrderBid": so_arbitrage_lead.ArbitrageOrderBid,
            "ArbitrageOrderAsk": so_arbitrage_lead.ArbitrageOrderAsk,
