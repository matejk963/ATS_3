...
from arbitrage_strategy.constants import BROKER_ID_1, BROKER_ID_2, INST_ID_1, INST_ID_2, INST_ID_3, PRODUCT_ID_1, PRODUCT_ID_2, \
    POWER_MONTHS, GAS_MONTHS, POWER_ITEM_ID, GAS_ITEM_ID
...
def create_feed(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        ...
        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_2, 0.1, COMMON.Direction.sell),
        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_1, 0.25, COMMON.Direction.sell),
        (POWER_MONTHS, INST_ID_2, POWER_ITEM_ID,
         BROKER_ID_1, 10.2, COMMON.Direction.sell),
        (GAS_MONTHS, INST_ID_3, GAS_ITEM_ID,
         BROKER_ID_1, 10.2, COMMON.Direction.sell),

        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_2, -0.1, COMMON.Direction.buy),
        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_1, -0.25, COMMON.Direction.buy),
        (POWER_MONTHS, INST_ID_2, POWER_ITEM_ID,
         BROKER_ID_1, 10, COMMON.Direction.buy),
        (GAS_MONTHS, INST_ID_3, GAS_ITEM_ID,
         BROKER_ID_1, 10, COMMON.Direction.buy),
        # (POWER_MONTHS, INST_ID_2, POWER_ITEM_ID_1, BROKER_ID_1, 76.03, COMMON.Direction.buy),


        # (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID_1, BROKER_ID_1, 76.1, COMMON.Direction.sell),
...
def create_trade_feed(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        ...
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_1, -0.2, COMMON.Direction.sell),
        (POWER_MONTHS, INST_ID_1, POWER_ITEM_ID,
         BROKER_ID_1, 0.2, COMMON.Direction.buy)
...
def create_trade_feed2(timestamp):
    ...
    for seq_id, instr_id, item_id, broker_id, price, direction in [
        ...
        # Gas: item id 20 -> Gas Yr 21 (OCT21-OCT22), # item id 21 -> "Gas Yr 22"
        (POWER_MONTHS, INST_ID_2, POWER_ITEM_ID,
         BROKER_ID_1, 10, COMMON.Direction.sell),
        (GAS_MONTHS, INST_ID_3, GAS_ITEM_ID,
         BROKER_ID_1, 10.2, COMMON.Direction.buy),
...
class SynthStratConfigTest(unittest.TestCase):
    def setUp(self):
        ...
        self.first_transfer = CETUTIL.utc_dt2ts(
            datetime.datetime(2022, 6, 27, 10, 0, 0))
        self.config = StrategyConfiguration(
            strategy_id=self.strategy_id,
            # can only place 2 instrument ids here
            instrument_ids=[INST_ID_1, INST_ID_2, INST_ID_3],
            strategies_folder=EXAMPLE_FOLDER,
            caption="Simple Arbitrage Strategy",
            package_name="arbitrage_strategy",
            product_ids=[PRODUCT_ID_1, PRODUCT_ID_1, PRODUCT_ID_2],
            broker_ids=[BROKER_ID_1, BROKER_ID_2],
            exchange_broker_ids=[BROKER_ID_1],
            broker_ids_main=[BROKER_ID_1, BROKER_ID_2],

            opt_margin=0.1,
            min_margin=0.07,
            max_margin=0.15,
            min_margin_aux=0.1,
            margin_for_closing=0.2,
            ...
            advanced_making=True,
            broker_making=True,
            market_volume_check_flag=1.0,
            ql_max=100000000000000000,

            maximum_neg_buyback=5,

            cons_making=True,
            cons_making_aux=False,
            cons_margin=0.06,
            cons_min_level=0.1,
    ...
    def _get_setup_limits(self):
        return {
            "limits_per_sequence": {"maximum_purchase_volume": {POWER_MONTHS: 100},
                                    "minimum_sales_price": {POWER_MONTHS: -100},
    ...
    def test(self):
        ...
        sim = Simulator(
            event_messages=[rest_strat_msg_create, limits_setup_message,
                            rest_strat_msg_activate],  # , so_register_msg
            json_feed=feed,
        ...
        traded_by_strategy = sim.get_net_traded_amounts_by_strategy(
            printout=True, only_traded=True)
