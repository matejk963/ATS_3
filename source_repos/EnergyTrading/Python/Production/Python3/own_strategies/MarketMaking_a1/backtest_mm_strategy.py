...
from mm_strategy.constants import (BROKER_ID, BROKER_ID_2, GAS_YEARS, GAS_YR22_ITEM_ID, TTF_ICE, TTF_PRODUCT_ID,
                                   ...
                                   DE_EEX, M1_PRODUCT_ID, M2_PRODUCT_ID, POWER_MONTHS, POWER_W37_ITEM_ID, POWER_WEEKS)
    ...
    return {"timestamp": timestamp,
            ...
            "data": [
                {"counter_party_ok": False, "system_rank": "487290945", "broker_id": broker_id, "txt": False,
                 ...
                 "is_tradable": True, "execution_restriction": "NON", "implied": False, "action": action,
...
def create_feed():
    ...
    with open(file_path_ba, 'r') as file:
        ...
        for i, row in enumerate(csv_reader):
            timestamp = datetime.datetime.strptime(row['timestamp'], '%Y-%m-%d %H:%M:%S').replace(year=2022,
                                                                                          month=6,
                                                                                          day=27) + dt.timedelta(hours=2)
            timestamp = timestamp.timestamp()
            counter = 1

            # for action in ['MOD', 'ADD']:
            #     counter = 1
            #     QUANTITY = 10
            #     # timestamp = row['timestamp']
            #     timestamp = datetime.datetime.strptime(new_row['timestamp'], '%Y-%m-%d %H:%M:%S').replace(year=2022,
            #                                                                                           month=6,
            #                                                                                           day=27) + dt.timedelta(hours=2)
            #     timestamp = timestamp.timestamp()
            #     if i == 0:
            ...
            buy_order = create_add_order(
                timestamp=timestamp,
                broker_id=broker_id,
                instrument_id=TTF_ICE,
                item_id=GAS_YR22_ITEM_ID,
                term_format_id=None,  # Adjust if needed
                sequence_id=GAS_YEARS,  # Adjust if needed
                direction=COMMON.Direction.buy,
                order_id=str(counter).zfill(8),  # Creates a zero-padded order_id
                price=-0.05,
                quantity=QUANTITY,
...
def create_trade_feed():
    ...
    with open(file_path_tr, 'r') as file:
        ...
        for i, new_row in enumerate(csv_reader):
            for action in ['ADD', 'MOD']:
                # counter = 1
                QUANTITY = 1

                timestamp = datetime.datetime.strptime(new_row[''], '%Y-%m-%d %H:%M:%S').replace(year=2022, month=6, day=27) + dt.timedelta(hours=2)
                timestamp = timestamp.timestamp()
                if action in ['ADD']:
                    action = 'MOD'
                    row = new_row
                ...
                trade_action = int(row['action'])  # Convert action to an integer
                ...
                if trade_action == 1:
                    ...
                    raise ValueError(f"Invalid action value: {trade_action}")
                ...
                if trade_action == -1:
                    direction = COMMON.Direction.buy
                    price2 = random.randrange(-10,-4,5)/100
                    price2 = -0.05
                elif trade_action == 1:
                    direction = COMMON.Direction.sell
                    price2 = random.randrange(5, 11,5)/100
                    price2 = 0.05
                else:
...
class SynthStratConfigTest(unittest.TestCase):
    def setUp(self):
        ...
        self.first_transfer = CETUTIL.utc_dt2ts(datetime.datetime(2022, 6, 27, 8, 0, 0))
        self.config = StrategyConfiguration(
            strategy_id=self.strategy_id,
            instrument_ids=[ITALY_BASELOAD, TTF_ICE],  # can only place 2 instrument ids here
            strategies_folder=EXAMPLE_FOLDER,
            caption="MarketMaking Synth Strategy",
            package_name="mm_strategy",
            product_ids=[ITALY_PRODUCT_ID, TTF_PRODUCT_ID],
            leg1_clip=1,
            leg2_clip=1,
            price_coeff=[1,1],
    ...
    def _get_setup_limits(self):
        return {
            "limits_per_sequence": {"maximum_purchase_volume": {GAS_YEARS: 1000, POWER_YEARS: 1000},
                                    "minimum_sales_price": {GAS_YEARS: -100, POWER_YEARS: -100},
    ...
    def test(self):
        ...
        feed.extend(create_trade_feed())
        feed = sorted(feed, key=lambda x: (x['timestamp'],x['data'][0]['order_id']))
        # feed = feed[:8000]


        # feed = create_feed2(self.first_transfer+5)
        # feed.extend(create_trade_feed2(self.first_transfer+15))
