...
class MarketMakingOrderAsk(SYB.SyntheticOrderBase):
    ...
    def act(self, localview, additional_views, timestamp):
        ...
        elif self.check_place_order(our_price) and strategy_stats.hold_bool(local_instrument_key.key):
            num_clips = abs(strategy_stats.net_clip_position[local_instrument_key.key])
            if num_clips < 1:
                quantity = round(num_clips * local_clip, 0)
            else:
                quantity = 0
            return self.create_slot(direction, quantity, our_price, info="MarketMakingLimitAskHold")
        else:
            return self.remove()
