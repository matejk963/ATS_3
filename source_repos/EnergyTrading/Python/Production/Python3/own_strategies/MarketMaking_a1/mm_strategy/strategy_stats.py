...
class StrategyStats(AdditionalTypeABC):
    ...
    def attributes_list(self):
        return ['strategy_id', 'instrument_list', 'trade_leg_dict', 'trade_spread_dict', 'price_leg_dict',
                ...
                'quote_bool_dict', 'hold_dict', 'pnl_dict', 'ql', 'ql_max', 'timestamp', 'margin_list', 'last_sl_timestamp']

    @property
    def attributes_list_ex(self):
        return ['strategy_id', 'trade_leg_dict', 'trade_spread_dict', 'price_leg_dict', 'price_spread_dict',
                'model_dict', 'pnl_dict', 'ql', 'timestamp', 'margin_list', 'last_sl_timestamp']
    ...
    @property
    def price_spread_dict(self):
        name_list = ['spread_bid', 'spread_ask', 'spread_mid']
        return {a: b for a,b in zip(name_list, [self.bid_spread, self.ask_spread, self.mid_spread])}
    ...
    def update_instruments(self, instrument_list, leg1_clip, leg2_clip, eql_w, eql_price, w1, w_std, quote_list,
        ...
        self.last_sl_timestamp = last_sl_timestamp
        self.timestamp = time.time()

    def set_margin_list(self, new_margin_list):
        self.margin_list = new_margin_list
    ...
    def update_strategy_prices(self, instrument_key, products, broker_id):
    ...
    def get_mid_std(self, mid_spread, burnout=50):
        self.mid_spread_count += 1
        self.mid_spread_sum += mid_spread
        self.mid_spread_sum_sq += mid_spread ** 2
        if self.mid_spread_count < burnout:
            std = 0.1
        else:
            std = ((self.mid_spread_sum_sq/self.mid_spread_count)-
                  (self.mid_spread_sum/self.mid_spread_count)) ** .5
        return std

    def update_model_prices(self, model_stats):
        if self.mid_spread:

            if self.mid_spread_prev is not None:
                std = self.get_mid_std(self.mid_spread)
                mid_chng = abs(self.mid_spread - self.mid_spread_prev)
                if mid_chng > 10 * std:
                    pass
                else:
                    self.mid_spread_prev = self.mid_spread
    ...
    def init_strategy_prices(self, products, broker_id, model_stats):
        ...
        for instkey in self.instrument_list:
            instrument_key = InstrumentKey().from_instrument_key(instkey)
            self.update_strategy_prices(instrument_key, products, broker_id)
            # for direction in direction_list:
