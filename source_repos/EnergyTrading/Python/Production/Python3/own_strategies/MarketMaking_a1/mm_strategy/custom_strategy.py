...
from .mm_lift_so import MarketMakingOrderLift
...
class CustomStrategy(SYSB.SyntheticOrderStrategyBase):
    ...
    reset_bool = False
    timestamp = None
    last_sl_timestamp = dt.datetime(1970, 1, 1, tzinfo=dt.timezone.utc).timestamp()
    ...
    def on_strategy_configuration_update(self, strategy_json):
        ...
        if not self.initialized:
            ...
            self.stats.update_instruments(instr_key_list, leg1_clip, leg2_clip, w, eql_price, w1, w_std, quote_list,
                                          price_coeff, self.stats.margin_list, self.stats.last_sl_timestamp)
            self.stats.init_strategy_prices(self.exchange.products, None, self.model)
            self.log_model_list = []
    ...
    def custom_on_order_book_update(self, orders, timestamp):
        ...
        for o in orders:
            ...
            if instrument_key.key in self.all_delivery_areas:
                inst_list.append(instrument_key)
        inst_list = list(dict.fromkeys(inst_list))
        if not inst_list:
            res = {}
        else:
            for instrument_key in inst_list:
                self.stats.update_strategy_prices(instrument_key, self.exchange.products, None)
                # if instrument_key.key in self.all_delivery_areas:
    ...
    def quote(self):
        ...
        self.stats.set_margin_list(margin_list)
    ...
    def check_band(self, band, bid_bool):
        if bid_bool:
            if (self.timestamp - self.last_sl_timestamp) < 7200:
                return band < self.stats.ask_spread
            else:
                return False
        else:
            if (self.timestamp - self.last_sl_timestamp) < 7200:
                return band > self.stats.bid_spread
            else:
                return False

    def adjust_band(self, old_band, bid_bool, adj_increment=0.05):
        if bid_bool:
            return old_band - adj_increment
        else:
            return old_band + adj_increment

    @property
    def open_size_increment_map(self):
        mapping_dict = {
            # Size_increment: band_multiple:
            0: 0,
            1: 1,
            2: 1.5
        }
    ...
    @property
    def close_size_increment_map(self):
        mapping_dict = {
            # Size_increment: band_multiple:
            0: 0,
            1: 0.5,
    ...
    def margin(self):
        ...
        if dt.datetime.fromtimestamp(self.stats.timestamp).time() >= dt.time(hour=17):
            if abs(self.stats.net_spread_position) < 0.01:
                return [None, None, None, None]
            else:
                if self.stats.net_spread_position > 0:
                    # if self.stats.bid_spread >= self.stats.model_price:
                    ask_band = self.stats.bid_spread - self.stats.model_price
                    return [None, ask_band, 0, 0]
                elif self.stats.net_spread_position > 0:
                    # if self.stats.ask_spread <= self.stats.model_price:
                    bid_band = self.stats.model_price - self.stats.ask_spread
                    return [bid_band, None, 0, 0]
        if self.model.is_std_model:
            if self.stats.is_model_std:
                margin = self.stats.model_std * self.stats.w_std
            else:
                return [None, None, None, None]
        else:
            margin = [None, None, None, None]
        # Check in which position tier we are now to adjust size_margin
        size_increment = abs(self.stats.net_spread_position)//3
        if size_increment == 3:
            margin = None
            size_margin = [0, 0]
        else:
            if size_increment == 0:
                ...
                band_multiple = self.open_size_increment_map[size_increment]
                bands = [self.stats.model_price - margin * band_multiple,
                         self.stats.model_price + margin * band_multiple]
                bands_check = [self.check_band(band,bid_bool)
                               for band,bid_bool in zip(bands, [True, False])]
                new_bands = [self.adjust_band(band, bid_bool) if check_bool else band
                             for band, bid_bool, check_bool in zip(bands, [True, False], bands_check)]
                size_margin = [new_band - self.stats.model_price for new_band in new_bands]
        if self.mtm_bool:
            if self.stats.net_spread_position == 0:
                close_margin = [False, False]
            else:

                sl_bands = [self.stats.model_price - margin * 3,
                            self.stats.model_price + margin * 3]
                # False if price is beyond sl_band
                sl_check = [self.check_band(sl_band, bid_bool)
                            for sl_band, bid_bool in zip(sl_bands, [True, False])]

                if any(sl_check):
                    self.stats.last_sl_timestamp = self.stats.timestamp
                    return [-margin * 3 if not a else None
                                    for a in sl_check] + [0, 0]
                else:
                    close_ratio = self.close_size_increment_map[size_increment]
                    close_band_margin = margin * close_ratio
                    close_bands = [self.stats.model_price - close_band_margin,
                                    self.stats.model_price + close_band_margin]
                    close_check = [self.check_band(close_band, bid_bool)
                                    for close_band, bid_bool in zip(close_bands, [True, False])]
                    close_margin = [-close_band_margin if a else False
                                    for a in close_check]

            # Side to make
            return [margin if not close else close
                     for close in close_margin] + size_margin
