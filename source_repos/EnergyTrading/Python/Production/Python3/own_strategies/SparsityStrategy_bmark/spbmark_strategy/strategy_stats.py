...
log = logging.getLogger("sparsity_bmark.strategy_statistics")
...
class StrategyStats(AdditionalTypeABC):
    ...
    def attributes_list(self):
        return ['strategy_id', 'caption', 'instrument_list','trade_leg_dict', 'trade_spread_dict', 'bool_dict', 'pnl_dict', 'aux_dict', 'param_dict', 'position_dict', 'lead_pers_dict', 'trade_decision_log']
    ...
    def log_trade_decision(self, trade_id, timestamp, decision_data):
        """
        ...
        self.trade_decision_log.append(decision_entry)
        
        # Keep only last 1000 entries to prevent memory issues
        if len(self.trade_decision_log) > 1000:
            self.trade_decision_log = self.trade_decision_log[-1000:]
        
        log.info(f"[TRADE_DECISION_LOG] Trade {trade_id}: {decision_data}")
