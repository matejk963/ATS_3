...
class ModelStats(object):
    ...
    def update_models(self, model_type, model_params):
        ...
        if model_type is None:
            ...
            self.model_dict['spread'] = MstdModel(model_params[0], 30, 2., 0, model_params[2], model_params[1], 0, False, False)
        elif self.model_type == 'MSTD_t':
            self.model_dict['spread'] = MstdModel(model_params[0], 30, 2., 0, model_params[2], model_params[1], 0, True, False)
        elif model_type == 'MID':
...
class MstdModel(ModelClass):
    ...
    def __init__(self, tau, N, p=2., value0=0, std0=0, tol=.025, burn=10, time_model=False, is_init=False):
        ...
        self.model = MSTD(tau / 2, N, p, value0, self._std0)
        if time_model:
            self.dt = self.dt_1
        else:
            self.dt = self.dt_x
    ...
    def initialize(self, value0):
        param_list = [self.model.tau, self.model._ma[0].n, self.model.p, value0, self._std0]
