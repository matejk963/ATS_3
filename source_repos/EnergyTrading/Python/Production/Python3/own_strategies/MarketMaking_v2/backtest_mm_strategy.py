...
def create_feed():
    ...
    with open(file_path_ba, 'r') as file:
        ...
        for row in csv_reader:
            ...
            timestamp = datetime.datetime.strptime(row['timestamp'], '%Y-%m-%d %H:%M:%S').replace(year=2022, month=6, day=27).timestamp()
...
def create_trade_feed():
    ...
    with open(file_path_tr, 'r') as file:
        ...
        for row in csv_reader:
            timestamp = datetime.datetime.strptime(row[''], '%Y-%m-%d %H:%M:%S').replace(year=2022, month=6, day=27).timestamp()
...
class SynthStratConfigTest(unittest.TestCase):
    ...
    def test(self):
        ...
        #feed = sorted(feed, key=lambda x: x['timestamp'])
