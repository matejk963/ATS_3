...
class IterativeSimulator(ATSIM.ExchangeSimulator):
    ...
    def receive(self, data_dict):
        ...
        for message in incoming_msgs:
            for o in message["body"]["data"]:
                if o.get("revision", None) is not None:
                    try:
                        p = self.exchange.products.get_by_id(o.get("product_id", None))
                    except COMMON.ProductNotFound:
                        p = None
                    if p is not None:
                        # In our simulation files, we don't allways guarantee the correct order of incoming messages.
                        # This is why here we make sure that the area revision of the order being passed to autotrader
                        # is at least as high as the last one that arrived for that same area
                        o["revision"] = max(
                            [p.orders._last_area_revision.get(o.get("delivery_area_id", None), 0), o["revision"] or 0]
                        )
    ...
    def send(self, data_dicts):
        ...
        to_log = []
        while data_dicts:
            to_log.append(("AT -> sim exchange", data_dicts))
            i += 1
            if i > 100:
                print("PROBLEM DETECTED. The following messages cause a very long (infinit?) loop "
                      "without moving forward in the exchange data feed.")
                for entry in to_log:
                    print("---- {} ----".format(entry[0]))
                    for msg in entry[1]:
                        print(msg.get("body", {}).get("message_type"), msg)
            ...
            to_log.append(("sim exchange -> AT", responses))
