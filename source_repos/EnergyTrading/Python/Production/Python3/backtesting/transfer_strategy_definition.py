...
def transfer(data, host=None, port=None, username="vt", password="vt", database_name="autoTRADER",
             num_child_processes=1, auth_source="autoTRADER"):
    ...
    mongodb = pymongo.MongoClient(host, port, username=username, password=password, authSource=auth_source)
    ...
    for object_type, strategy_objects in six.iteritems(strategies):
        for strategy_int_num, strategy in six.iteritems(strategy_objects):
            if strategy is None:
            ...
            if object_type == "strategy_objects":
                if "child_id" not in strategy:
                    if six.PY2:
                        ...
                        strategy["child_id"] = (
                            int(hashlib.sha256(strategy_int_num.encode('utf-8')).hexdigest(), 16)
                            % nb_child if nb_child != 0 else 0
                        )
            # we need to convert numpy int values to python int before writing to mongo
            strategy = PY2LIB.convert_dict_with_numpy_values(strategy)
