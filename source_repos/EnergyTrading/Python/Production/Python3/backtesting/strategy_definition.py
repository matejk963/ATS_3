...
class _NumpyTSFrame(object):
    ...
    def __init__(self, valid_from, valid_to, ts_raster, strategy_columns, userdef_columns):
        ...
        self.dtype = np.dtype(
            [("start", int), ("end", int)]
            + [(value_name, float) for value_name in self.columns]
        )
...
class StrategyDefinitionBase(object):
    ...
    def __init__(
        ...
        for ts_name in self.ts_names + self.algo_ts_names + self.limits_ts_names + self.user_ts:
            ...
            for begin, end, value in [(v["begin"], v["end"], v["value"]) for v in ts_values]:
                self.fill(ts_name, value, int(begin), int(end), overwrite=True)

        for ts_name, ts_values in six.iteritems(kwargs.get("timeseries", {})):
    ...
    def set_default_limits(self):
        ...
        for ts_name, ts_values in six.iteritems(DEFAULT_LIMIT_VALUES):
...
def zip_folder(folder):
    ...
    :rtype: str
    """
    ziparchive = six.BytesIO()
    with contextlib.closing(zipfile.ZipFile(ziparchive, "w")) as archive:
        for file in os.listdir(folder):
            if file.endswith(".py"):
                readmode = "r" if six.PY2 else "rb"
                with open(os.path.join(folder, file), readmode) as fl_content:
