...
class Simulator(FLOG.BacktestingComplianceLoggerMixin):
    ...
    @staticmethod
    def _close_order_book_exchange(exchange, timestamp):
        """Close all the order book of one specific exchange."""
    ...
    def _close_order_book(self, exchange_id, timestamp):
        """Close all the order book of the autotrader and autotrader_child exchanges.
        :param exchange_id: exchange ID. One of "TRAYPORT", "EPEX" or "NORD".
                            Not needed if only 1 exchange is simulated.
        :type exchange_id: str
        """
        autotrader_exchange = self.autotrader.get_exchange(exchange_id)
        autotrader_child_exchange = self.autotrader_child.get_exchange(exchange_id)
        for exchange in (autotrader_exchange, autotrader_child_exchange):
            self._close_order_book_exchange(exchange=exchange, timestamp=timestamp)
            ...
            with zipfile.ZipFile(filepath, "r") as myzip:
                for zipfilecontent in myzip.filelist:
                    if zipfilecontent.filename.endswith(".json"):
                        for data in json.loads(myzip.read(zipfilecontent)):
                            yield data
    ...
    def run(
        ...
        **kwargs
        ...
        if self.json_feed is None:
            ...
            if isinstance(self.feed_paths, six.string_types):
        ...
        if COMMON.Exchange.trayport in self.exchange_simulators:
            ...
            assert not child_trayport.is_parent
            child_trayport.update_synthetic_products(self.autotrader_child.current_timestamp)
        if COMMON.Exchange.epex in self.exchange_simulators:
            self.autotrader.epex.init_files_correlation_ids = {"initialized": True}
            self.autotrader_child.epex.init_files_correlation_ids = {"initialized": True}
        if COMMON.Exchange.nordpool in self.exchange_simulators:
            self.autotrader.nordpool.init_files_correlation_ids = {"initialized": True}
            self.autotrader_child.nordpool.init_files_correlation_ids = {"initialized": True}
        ...
        def package_strategy(strategy):
            if six.PY2:
                # python 2 returns a string here
                strategy["package"] = base64.b64encode(strategy.get("package", ""))
            else:
                # python 3 returns bytes, it has to decode it to a string, otherwise it cannot be dumped into a json
                strategy["package"] = base64.b64encode(strategy.get("package", b"")).decode()
            return strategy
        ...
        if self.save_results:
            ...
            # needed because in Python3 we cannot dump int32
            class NumpyIntEncoder(JSONEncoder):
                # Override the default method to transform numpy int32 to int
                def default(self, obj):
                    if isinstance(obj, (int, np.int32)):
                        return int(obj)

            with open(results_path, "w") as f:
                json.dump(results, f, cls=NumpyIntEncoder)
                print("----------------------------------")
    ...
    def _plot_omt(self):
        ...
        internal use.
        """
        if COMMON.Exchange.epex not in list(self.exchange_simulators.keys()):
        ...
        if msg["message_type"] == "new_session":
            ...
            if exchange_id is None:
                ...
                for exchange in {strat.exchange for strat in self.autotrader.strategies.values()}:
                    self._close_order_book(exchange.internal_id, timestamp)
            else:
                print("Closing all order books on exchanges due to new session.")
                self._close_order_book(exchange_id, timestamp)
    ...
    def _initialize_trayport(self, feed_start_time):
        ...
        as synthetic products, based on sequences and instruments.
        For this reason, we have to properly initialize the exchange here.
    ...
    def export_market_data(self, trade_type,
        ...
        if exchange_id is None:
            if len(self.exchange_simulators) == 1:
                exchange_id = list(self.exchange_simulators.keys())[0]
            else:
        ...
        if self.use_persistence:
            ...
            csv_columns = MDE._get_csv_headers(
                stats,
    ...
    def get_trade_stats(self, trades, is_own=True):
        ...
        array_type = np.dtype([('execution_time', 'f8'), ('delivery_start', int), ('delivery_end', int),
    ...
    def get_net_traded_amounts_by_strategy(self, printout=False, only_traded=False):
        ...
        columns_header = ("|                     Strategy                      |"
                          ...
                          "      product-type     |"
                          "                             product-name                              |")
        lengths = list(map(len, columns_header.split("|")))[1:]
        if printout:
