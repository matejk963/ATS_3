...
def generate_file_paths(db, country, start_date_obj, end_date_obj, data_type='power'):
    ...
    base_path = r'market_data/natgas'
    derivative_path = 'spot/csv'
    file_paths = []
    country = country.replace('_spot','')
    ...
    while current_date <= end_date_obj:
        ...
        file_name = 'GasSpotMarketResults_' + country.upper() + '_' + f"{date_str}.csv"
...
def clean_data(df):
    ...
    value_B8 = df.iloc[3, 1]  # B8 in the original CSV (considering header row now)
    ...
    df = df[df.iloc[:, 0].str.contains('SP', na=False)]

    
    # Retain only the specified columns
    required_columns = ['Market Area', 'Long Name', 'Delivery Start', 'Delivery End',
                        'Settlement Price']
    ...
    df['delivery'] = 'Base'
    df['product_type'] = df['Long Name'].str.extract(r'(DAY|WEEKEND|SAT|SUN|WITHIN-DAY)')
    df['product_type'] = df['product_type'].str.capitalize()
    # Step 2: Apply the fallback for rows where 'product_type' is NaN
    # df.loc[df['product_type'].isna(), 'product_type'] = df['Product'].apply(lambda x: long_name_prod_dict.get(x[-1]) if isinstance(x, str) else x)
    
    # Drop 'Product' and 'Long Name' columns
    df.drop(columns=['Market Area', 'Long Name'], inplace=True)
...
if __name__ == "__main__":
    ...
    for country in ['ttf_spot', 'ttf_spot', 'ttf_spot']:
