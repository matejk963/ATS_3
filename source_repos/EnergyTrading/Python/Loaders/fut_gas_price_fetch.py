...
def generate_file_paths(db, country, start_date_obj, end_date_obj, data_type='power'):
    ...
    while current_date <= end_date_obj:
        ...
        file_name = 'GasFutureMarketResults_' + country.upper() + '_' + f"{date_str}.csv"
...
def clean_data(df):
    ...
    df = df[df.iloc[:, 0].str.contains('PR', na=False)]
    df = df[~df.iloc[:,2].str.contains('OTF', na=False)]
    
    # Retain only the specified columns
    required_columns = ['Product', 'Long Name', 'Delivery Start', 'Delivery End',
                        'Settlement Price', 'Open Price', 'High Price',
                        'Low Price', 'Last Price', 'Traded Lots',
                        'Open Interest Lots']
    ...
    df['delivery'] = 'Base'
    df['product_type'] = df['Long Name'].str.extract(r'(Day|Weekend|Week|Month|Quarter|Year|Season)')
