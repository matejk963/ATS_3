...
def generate_file_paths(db, country, start_date_obj, end_date_obj, data_type='power'):
    ...
    base_path = r'market_data/environmental'
    ...
    while current_date <= end_date_obj:
        ...
        file_name = 'EmissionFutureMarketResults_' + f"{date_str}.csv"
        file_path = f"{base_path}/{derivative_path}/{year}/{date_str}/" + file_name
...
def clean_data(df):
    ...
    df = df[~df.iloc[:,2].str.contains('OTF', na=False)].copy()
    df = df[df.iloc[:,1].str.contains('FEUA', na=False)].copy()
    ...
    required_columns = ['Product', 'Long Name', 'Delivery Start', 'Delivery End',
                        ...
                        'Open Interest Lots']
      # Set the header row as the columns
