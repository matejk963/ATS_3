...
class AvCapFetch:
    ...
    def determine_value_date_range(self, forecast_date):
        forecast_date = pd.to_datetime(forecast_date.date())
        
        if self.date_range_type == 'day_ahead':
            self.value_start_date = (forecast_date + pd.DateOffset(days=1)).replace(hour=0, minute=0)
            self.value_end_date = self.value_start_date + pd.DateOffset(days=1) - pd.DateOffset(minutes=1)
            
        elif self.date_range_type == 'week_ahead':
            # Calculate the next Monday from the forecast_date
            days_until_next_monday = 7 - forecast_date.weekday() 
            next_monday = forecast_date + pd.DateOffset(days=days_until_next_monday)
            
            self.value_start_date = next_monday.replace(hour=0, minute=0)
            self.value_end_date = next_monday + pd.DateOffset(weeks=1) - pd.DateOffset(minutes=1)
            
        else:
            raise ValueError("Unsupported date_range_type!")
        
        self.value_date_range = pd.date_range(start=self.value_start_date, 
                                              end=self.value_end_date, 
                                              freq='H')

    def load_data(self):
        required_files = self.determine_files()
        self.dfs, self.downloaded_files = self.retrieve_data_from_ftp(required_files)

    def prepare_data(self):
        if self.dfs is None or self.downloaded_files is None:
            self.load_data()
        df_combined = pd.concat(self.dfs, ignore_index=True)
        df_combined = df_combined.loc[df_combined['Id']==self.asset_id].copy()
        df_combined = df_combined.drop_duplicates(subset=['ForecastDate',
                                                          'ValueDate',
                                                          'Value'])
        ...
        #Correction
        df = df.loc[((df['ForecastDate']>=self.forecast_start_date)
                     &(df['ForecastDate']<(self.forecast_end_date+
        ...
        df['ForecastMinutes'] = df['ForecastDate'].dt.hour * 60 \
            + df['ForecastDate'].dt.minute
        df = df[df['ForecastMinutes']<=self.cutoff_minutes].copy()

        max_values = df.groupby('ForecastDateOnly')['ForecastMinutes'].transform('max')
        result = df[df['ForecastMinutes']==max_values].copy()

        result['days'] = (result['ValueDateOnly']-result['ForecastDateOnly']).dt.days
        result['day_ahead'] = np.where(result['days']==1,1,0)
        result['weekend_ahead'] = np.where((((result['days']+
                                       result['ForecastDateOnly'].dt.weekday)>=5)&
                                      ((result['days']+
                                        result['ForecastDateOnly'].dt.weekday)<=6)),
                                      1,0)
        result['week_ahead'] = np.where((((result['days']+
                                       result['ForecastDateOnly'].dt.weekday)>=7)&
                                      ((result['days']+
                                        result['ForecastDateOnly'].dt.weekday)<=13)),
                                      1,0)

        self.prepared_data = result.copy()
    ...
    def to_hourly_series(self, date_range_type='day_ahead'):
        self.date_range_type = date_range_type
        
        self.prepare_data()
        return self._to_hourly_series(self.prepared_data)
        
        

    def _to_hourly_series(self, df_in):
        df = df_in.copy()  # To ensure we don't modify the input dataframe
        df['filter_value'] = np.where(((df[self.date_range_type]<
                               df[self.date_range_type].shift(-1))|
                                      (df[self.date_range_type]==1)),df['Value'],np.nan)
