...
try:
    ...
    from flow_spot_daily_improved import spot_daily_improved  # Use improved version
    ...
    try:
        ...
        if "UP" in result.stdout or is_port_in_use(4200):
            ...
            if is_port_in_use(4200):
                ...
                os.system("taskkill /F /IM prefect.exe")
...
async def reset_work_pools():
    ...
    try:
        ...
        if result.stdout and WORK_POOL_MATEJ in result.stdout:
            ...
            subprocess.run(["prefect", "work-pool", "delete", WORK_POOL_MATEJ, "--force"], check=True)
        else:
    ...
    subprocess.run(["prefect", "work-pool", "create", WORK_POOL_MATEJ, "--type", "process", "--overwrite"], check=True, input="y\n", text=True)
    ...
    try:
        ...
        if result.stdout and WORK_POOL_FUND in result.stdout:
            ...
            subprocess.run(["prefect", "work-pool", "delete", WORK_POOL_FUND, "--force"], check=True)
        else:
    ...
    subprocess.run(["prefect", "work-pool", "create", WORK_POOL_FUND, "--type", "process", "--overwrite"], check=True, input="y\n", text=True)
    ...
    try:
        ...
        if result.stdout and WORK_POOL_DEFAULT in result.stdout:
            ...
            subprocess.run(["prefect", "work-pool", "delete", WORK_POOL_DEFAULT, "--force"], check=True)
        else:
    ...
    subprocess.run(["prefect", "work-pool", "create", WORK_POOL_DEFAULT, "--type", "process", "--overwrite"], check=True, input="y\n", text=True)
...
async def main():
    ...
    await reset_work_pools()
    await asyncio.sleep(5)
    await delete_existing_deployments()

if __name__ == "__main__":
    os.environ["PREFECT_API_URL"] = "http://127.0.0.1:4200/api"    # Delete existing deployments before deploying new ones
    asyncio.run(main())
    ...
    # Mid forecast percentiles fundamentals fetch EC Ens12
    fund_fetch.deploy(
    ...
    #     ],
    #     entrypoint_type=EntrypointType.MODULE_PATH
    # )# Replace the spot-daily deployment with the improved version
    try:
        spot_daily_improved.deploy(
            name="spot-daily-improved",  # New name to distinguish from original
            ...
            schedules=[
                # Keep all 3 original schedules but with improved resource management
                DeploymentScheduleCreate(
        ...
        print("✅ spot-daily-improved deployment successful!")
    except Exception as e:
        print(f"❌ ERROR deploying spot-daily-improved: {e}")
    ...
    print("🎉 All deployments completed with improved resource management!")
