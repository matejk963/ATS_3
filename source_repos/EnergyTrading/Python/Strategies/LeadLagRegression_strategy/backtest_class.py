...
class BacktestLL(BacktestClass):
    ...
    def simulate_strategy(self, strategy_class, instr, df_lag, df_lead):
        ...
        df_lag=calculate_MACD(df_lag)
        #df_lag=calculate_lead_lag_triggers(df_lead, df_lag, max_secs_between_trades, cluster_trade_num_threshold, min_price_movement)
        ...
        df_lag= pd.concat([df_lag, df_lead[['datetime', 'timestamp', 'tag', 'trd_price', 'lag_price_predicted', 'lag_price_predicted_datetime', 'lag_price_tick', 'coef1', 'coef2', 'lead_price_tick']].reset_index(drop=True)]).reset_index(
            drop=True).sort_values('datetime')

        # Convert the timestamp to date and set it as a separate column if not already done
        df_lag['date'] = df_lag['timestamp'].dt.date
        ...
        #df_lag['lag_price_predicted'] = df_lag.groupby('date')['lag_price_predicted'].ffill()
