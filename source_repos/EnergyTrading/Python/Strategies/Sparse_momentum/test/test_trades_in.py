...
from Strategies.Sparse_momentum.ob_attributes import Attributes, TR_attributes
...
tenor_list = ['m', 'm', 'q', 'q', 'y', 'y', 'm', 'q']
tn1_list = [1, 2, 1, 2, 1, 2, 1, 1]
# mkt_list = ['de']
# tenor_list = ['w']
tn1_list = [1]
tn2_list = []
...
data_class_db = TPData()
data_class_tp = TPDataDa()
data_class_db.create_connection('PostgreSQL')

tr_att = TR_attributes([])
for k, ds in enumerate(dates):
    ...
    for i, (m, t, n, pd1, pd2) in enumerate(zip(mkt_list, tenor_list, tn_list,
        ...
        try:
            ...
            if is_db:
                df_ba = data_class_db.get_best_ob_data(m, t, venue_list, pd1, bT, eT, prod, None)
                if df_ba.empty:
                    df_ba = data_class_tp.get_best_ob_data(m, t, venue_list, pd1, bT, eT, prod, None)
            else:
                df_ba = data_class_tp.get_best_ob_data(m, t, venue_list, pd1, bT, eT, prod, None)
            df_ba = df_ba.rename(columns={'bidbestprice': 'b_price', 'askbestprice': 'a_price'})
            df_tr = data_class_tr.clean_trades(df_tr, df_ba)
        except(Exception):
            continue
        df_tr['count'] = 1
    ...
    try:
        ...
        pd_tr = pd.concat([pd_tr, tr_dicts[2].reindex(ts)])
    except(AttributeError):
        continue
    
    
    
train_data = pd_tr[pd_tr.index.date<dt.date(2024,7,10)].copy()
test_data = pd_tr[pd_tr.index.date==dt.date(2024,7,10)].copy()
