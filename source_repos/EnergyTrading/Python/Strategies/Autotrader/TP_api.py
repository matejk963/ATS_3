...
from Strategies.Autotrader.environment import TP_CRED
...
class TP_api():
    ...
    def get_monitoring_for(self, algo_id, table):
        # Prepare the datetime strings
        today = datetime.now() - timedelta(days=15)
        tomorrow = today + timedelta(days=30)
        utc_iso_from = datetime(today.year, today.month, today.day, 0, 0, 0,
    ...
    def get_current_pnl(self, day=None, manual=False):
        today = datetime.now()
        if day:
