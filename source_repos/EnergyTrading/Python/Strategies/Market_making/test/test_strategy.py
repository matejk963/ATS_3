...
def calc_ema_m_std(df_data, tau, margin, w, w1, w_std, eql_p,N=30, tol=0):
    ...
    for day, group in df_data.groupby(pd.Grouper(freq='B')):
        mid_ser = .5 * (group.loc[:, 'bid'] + group.loc[:, 'ask'])
        mid_list = mid_ser.values
        model = simple_model(MSTD(tau, N), [tau / 2, N, 2, mid_list[0], .2],
...
param_dict['t_end'] = datetime(2025, 3, 28)
param_dict['take_profit'] = 0.3
param_dict['stop_loss_rat'] = 1.
param_dict['ba_spread'] = 0.50
param_dict['ba_max'] = 1.5
param_dict['br_fee'] = 0.08
...
ax = data_dict['m1xq1'].iloc[:,:2].plot(grid=True, legend=True, figsize=(34, 22))
data_dict['m1xq1'].iloc[:,3].plot(grid=True, legend=True, style='.', figsize=(36, 24), ax=ax)
data_dict['output'].plot(grid=True, legend=True, figsize=(34, 22), ax=ax)
