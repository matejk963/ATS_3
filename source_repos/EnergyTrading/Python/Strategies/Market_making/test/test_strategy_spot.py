...
def calc_ema_mstd_spot_model(df_data, tau, margin, w, w1, df_model, tol=0):
    ts = df_data.index
    df_model = df_model.reindex(ts).ffill()
    bands = []
    for day, group in df_data.groupby(pd.Grouper(freq='B')):
...
ax = data_dict['m1x2'].iloc[:,:2].plot(grid=True, legend=True, figsize=(34, 22))
data_dict['m1x2'].iloc[:,3].plot(grid=True, legend=True, style='.', figsize=(36, 24), ax=ax)
