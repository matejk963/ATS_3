...
# Weekly model data
import pickle, os
folder_path = r'Z:\Data\Spot\Model\Backtest\ModelPrices'
file_name = 'de_w1_w2_w3_20240401_20240528_mean_percentiles.pickle'
file_path = os.path.join(folder_path, file_name)
with open(file_path, 'rb') as f:
    weekly_model_dict = pickle.load(f)
weekly_model_df = weekly_model_dict['de'][1]
...
model_price_df = df_ba.join(weekly_model_df, on='date')
...
# Adjust trades
df_em = calc_ema_mstd_spot(df_ba, tau, margin, w, w1, model_price_df).dropna()
...
data_dict['w1x2'].iloc[:, 3].dropna().plot(ax=ax, grid=True, legend=True, style='.')
