...
def calculate_volatility(df_lag, window=20, price_col='trd_price'):
    """
    Calculates short-term rolling volatility of trade prices per day.

    Parameters:
    - df_lag: DataFrame with at least ['datetime', 'trd_price'] columns.
    - window: Rolling window size (in number of ticks) for std dev.
    - price_col: Column to use for volatility calculation (default = 'trd_price').

    Returns:
    - Original DataFrame with a new 'VOL_{window}' column.
    """
    # Extract date part for grouping
    df_lag['date'] = df_lag['datetime'].dt.date

    # Keep only rows with valid trade prices
    df_filtered = df_lag.dropna(subset=[price_col])

    # Function to compute volatility (rolling std)
    def calculate_vol(group):
        group = group.sort_values('datetime')  # sort in case it's not
        group[f'VOL_{window}'] = group[price_col].rolling(window=window, min_periods=1).std()
        return group

    # Apply volatility calculation by day
    df_filtered = df_filtered.groupby('date').apply(calculate_vol)

    # Merge results back and forward-fill within date
    df_lag = df_lag.merge(df_filtered[['datetime', f'VOL_{window}']].drop_duplicates('datetime', keep='last'),
                          on='datetime', how='left')
    df_lag[f'VOL_{window}'] = df_lag.groupby('date')[f'VOL_{window}'].ffill()
