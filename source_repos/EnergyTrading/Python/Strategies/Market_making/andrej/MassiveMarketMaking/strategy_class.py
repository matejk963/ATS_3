...
class StrategyLL(StrategyClass):
    ...
    def process(self, price_dict, volm_class, trade_type):
        ...
        self.MACD=price_dict['MACD_12_26']
        self.volatility=price_dict['VOL_20']


        # Check if need to close position
        if self.timestamp_.hour==10 and self.timestamp_.minute==31 and self.timestamp_.day>=2 and self.timestamp_.month==1:#and self.tag=='lead'
            pass #print("Jozko")

        if True: #execute lead opening logic when not in position
            price, vol = self.act_lead_order()

        if self.curr_position != 0 and vol==0:                       #execute lift closing logic when already in position
...
################################################################################################################
    ...
    def calculate_open_slot(self):
        ...
        kappa= self.param_dict['kappa']
        A=self.param_dict['A']
        q=self.curr_position
        sigma_2=self.volatility

        self.bid_spread=(1/gamma)*np.log(1+gamma/kappa)+(q+1/2)*np.sqrt((sigma_2*gamma*(1+gamma/kappa)**(1+kappa/gamma))/(2*kappa*A))
        self.ask_spread=(1/gamma)*np.log(1+gamma/kappa)+(-q+1/2)*np.sqrt((sigma_2*gamma*(1+gamma/kappa)**(1+kappa/gamma))/(2*kappa*A))


        if self.timestamp_.time() <= self.param_dict['t_end'] and not np.isnan(self.trd_p):

            if self.trd_p <= mid_price-self.bid_spread: # and self.lag_price_predicted>self.lag_price_tick
                action = 1
                price = round(mid_price-self.bid_spread,2)

            if self.trd_p >= mid_price+self.ask_spread: # and self.lag_price_predicted>self.lag_price_tick
                action = -1
                price = round(mid_price+self.ask_spread,2)
