...
# import cx_Oracle
# cx_Oracle.init_oracle_client(lib_dir= r"C:\Users\algouser\Oracle\instantclient_19_9")
...
def prepare_data_combined(LoB_dicts, tr_dicts, trAtt, depth_list, period_list,
    ...
    idx, csum, ts = obAtt.calc_tick_index(ob_dict[ts_lag], None, None, 1)
    print("Prepare reg data mkt")
    ob_prim = obAtt.prepare_reg_data_mkt(ob_dict[ts_lag], ts, idx, period_list,
                                         depth_list, ts_data=timestamp, keep_ts=True)
     
    # trades attributes
    print("Prepare trd reg data")
    tr_prim = trAtt.prepare_reg_data(tr_dicts[ts_lag], timestamp, [], period_list)
    return tr_prim, ob_dict, tr_dicts, pd.concat([ob_prim, tr_prim], axis=1)
...
if __name__ == '__main__':
    ...
    # ------------------ dataset prep ---------------------------------
    args_config = {
        '--instrument': {'type': str, 'default': 'frm1'},
    ...
    inst_trades = data_class.get_trades_inst(mkt_list[0], venue_list, start_date, end_date, prod='base', spread_bool=False)
    ...
    df_data = pd.concat([df_data, inst_trades], axis=1).reset_index(names='timestamp')
    
    df_ba = pd.DataFrame(all_ob_dicts[ts_lag]).set_index('timestamp')
    ts_new = df_data.set_index('timestamp').index.union(df_ba.index)
    df_reg = df_data.copy()
    ...
    df_backtest = pd.concat([df_ba_, df_reg[concat_columns].set_index('timestamp')], axis=1)
