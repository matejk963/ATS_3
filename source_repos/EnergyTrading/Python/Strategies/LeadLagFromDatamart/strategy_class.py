...
class StrategyLL(StrategyClass):
    ...
    def process(self, price_dict, volm_class, trade_type):
        ...
        self.MACD_6_14 = float(price_dict['MACD_6_14'])
        self.MACD_12_26 = float(price_dict['MACD_12_26'])
        self.MACD_18_38 = float(price_dict['MACD_18_38'])
        self.MACD_30_60 = float(price_dict['MACD_30_60'])
        #self.VII_lead_3 = price_dict['VII_lead_3']
        #self.VII_lead_5 = price_dict['VII_lead_5']
        #self.VII_lead_10 = price_dict['VII_lead_10']
        self.VII_lag_3 = float(price_dict['VII_lag_3'])
        self.VII_lag_5 = float(price_dict['VII_lag_5'])
        self.VII_lag_7= float(price_dict['VII_lag_7'])

        #self.VII_3_ratio = price_dict['VII_3_ratio']
        #self.VII_5_ratio = price_dict['VII_5_ratio']
        #self.VII_10_ratio = price_dict['VII_10_ratio']
        ...
        self.lag_price_predicted = float(price_dict['ll_aux_ens_dem2_fair_price'])
        self.lag_price_tick = float(price_dict['lag_price_tick'])
        #self.lead_price_tick = price_dict['lead_price_tick']

        #self.coef1 = price_dict['coef1']
        #self.coef2 = price_dict['coef2']

        #self.XGB_SCORE_LONG=price_dict['XGB_SCORE_LONG']
        self.XGB_SCORE_SHORT=price_dict['p_short_xgb']


        # Check if need to close position
        if self.timestamp_.hour==16 and self.timestamp_.minute==8 and self.timestamp_.day>=7 and self.timestamp_.month==3 and self.tag=='lead':#and self.tag=='lead'
            print("Jozko")
...
################################################################################################################
    ...
    def act_lead_order(self):
        ...
        if  self.tag!='lag' and ba_spread <= self.param_dict['ba_max']: #and self.timestamp_.hour<16 and self.timestamp_.date() not in self.high_volumes  and self.coef2>=1  and self.coef2<=0.725
...
################################################################################################################
    ...
    def calculate_open_slot(self):
        ...
        if self.timestamp_.time() <= self.param_dict['t_end'] and self.lag_price_predicted:
            ...
            #     'MACD_6_14': [self.MACD_6_14],
            #     'MACD_12_26': [self.MACD_12_26],
            #     'MACD_18_38': [self.MACD_18_38],
            #     'VII_lead_3': [self.VII_lead_3],
            #     'VII_lead_5': [self.VII_lead_5],
            #     'VII_lead_10': [self.VII_lead_10],
            #     'VII_lag_3': [self.VII_lag_3],
            #     'VII_lag_5': [self.VII_lag_5],
            #     'VII_lag_10': [self.VII_lag_10],
            #     'VII_3_ratio': [self.VII_3_ratio],
            #     'VII_5_ratio': [self.VII_5_ratio],
            #     'VII_10_ratio': [self.VII_10_ratio],
            #
            #     'price_diff': [(self.lag_price_predicted-self.ask_p)],
            #     'ba_spread': [self.ba_spread]
            # })
            #
            # # Example production data
            ...
            # Make predictions
           #production_prediction_long = self.XGB_SCORE_LONG
            production_prediction_short = self.XGB_SCORE_SHORT

            if production_prediction_short>0.9 and (self.lag_price_predicted - self.bid_p) < self.param_dict['price_diff_short_threshold']: # and self.lag_price_predicted>self.lag_price_tick
                action = -1
                price = self.bid_p

            # if not self.param_dict['combined_mode']:
            #     if self.MACD_12_26 > self.param_dict['MACD_long_threshold']  and (self.lag_price_predicted - self.ask_p) > self.param_dict['price_diff_long_threshold'] and self.lag_price_predicted>self.lag_price_tick and abs(self.VII_lag_5)>self.param_dict['minimum_intensity']: # and self.lag_price_predicted>self.lag_price_tick
            #         action = 1
            #         price = self.ask_p
            #     elif self.MACD_12_26 <= self.param_dict['MACD_short_threshold'] and (self.lag_price_predicted - self.bid_p) < self.param_dict['price_diff_short_threshold'] and self.lag_price_predicted<self.lag_price_tick and abs(self.VII_lag_5)>self.param_dict['minimum_intensity']: # and self.lag_price_predicted<self.lag_price_tick and self.MACD_30_60<-0.05
            #         action = -1
            #         price = self.bid_p
            #
            # else:
            #     if self.MACD_12_26 and self.lag_price_predicted and (self.MACD_12_26 + (
            #             self.lag_price_predicted - self.ask_p)) >self.param_dict['combined_long_threshold'] and self.VII_lag_5>1 and self.VII_lead_5>1:# and self.lag_price_predicted>self.lag_price_tick:  # not production_data_long.isnull().values.any() and production_prediction_long>self.param_dict['long_threshold']:
            #         action = 1
            #         price = self.ask_p
            #     elif self.MACD_12_26 and self.lag_price_predicted and (-self.MACD_12_26 - (
            #             self.lag_price_predicted - self.bid_p)) > self.param_dict['combined_short_threshold'] and self.VII_lag_5>1 and self.VII_lead_5>1:# and self.lag_price_predicted<self.lag_price_tick:  # not production_data_short.isnull().values.any() and production_prediction_short>self.param_dict['short_threshold']:
            #         action = -1
            #         price = self.bid_p
