...
data_min = bid_train_data.resample('H').count()
times_dif = round(bid_train_data.index.to_series().diff().dt.total_seconds(),1).dropna()
counts = times_dif.groupby(times_dif).count()
plt.figure(figsize=(8, 6))  # Set the figure size
counts.plot(kind='bar')  # Bar plot of the counts
plt.xlabel('Category')
plt.ylabel('Count')
