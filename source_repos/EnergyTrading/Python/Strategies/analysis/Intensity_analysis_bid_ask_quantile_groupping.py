...
def calculate_prediction_path(
    ...
    purchase_history = resampled_data['transactions'].values
    quantile_history = resampled_data[perc_col].values
    
    customer_history = resampled_data.copy()
    ...
    # Assuming resampled_data is your DataFrame and model_dict is your dictionary of models
    customer_history = resampled_data.copy()
    
    # Reset counts each minute
    grouped = customer_history.groupby(pd.Grouper(freq='T'))  # 'T' represents minute
    
    model_data = pd.DataFrame()
    for _, group in grouped:
        if not group.empty:
            # Add T column
            group["T"] = np.arange(group.shape[0])
            # Add transactions column
            group["transactions"] = group["transactions"].apply(lambda t: int(t > 0))
            # Calculate cumulative transactions
            group["frequency"] = group["transactions"].cumsum()
            # Add recency column
            group["recency"] = group.apply(
                lambda row: row["T"] if row["transactions"] != 0 else np.nan, axis=1
            )
            group["recency"] = group["recency"].fillna(method="ffill").fillna(0)
    
            # Apply model based on quantile
            
            group['model_output'] = group.apply(apply_model, args=(model_dict['bid_trade'], t), axis=1)
            
            if model_data.empty:
                model_data = group.copy()
            else:
                model_data = pd.concat([model_data, group])
