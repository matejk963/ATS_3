# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 17:11:29 2024
...
# Step 1: Project Anomalies onto the EOFs (20 components)
def project_onto_eofs(data_region, eofs):
    ...
    # Normalize the geopotential height anomalies
    ...
    projection = np.dot(anomalies, eofs.values.reshape(eofs.shape[0], -1).T)
    
    return projection

# Step 2: Calculate Projections onto Regime Centroids
def project_onto_regimes(projections, centroids):
    ...
    # For each regime, calculate the mean and std of the projections
    regime_means = np.mean(projected_regimes, axis=0)
    regime_stds = np.std(projected_regimes, axis=0)
    
    # Normalize by the mean and std for each regime
    normalized_projections = (projected_regimes - regime_means) / regime_stds
    
    return normalized_projections

# Step 3: Calculate Normalized Projection by Comparing with Centroids
def normalize_projections(projections, centroids):
    # Compute the projection distance (Euclidean distance) between the projected PCs and regime centroids
    distances = cdist(projections, centroids)
    
    # Normalize by the standard deviation of each regime's projection
    normalized_projections = (projections - centroids.mean(axis=0)) / centroids.std(axis=0)
    
    return normalized_projections

# Compute regime strength and probabilities
regime_strength = calculate_regime_strength(pcs, centroids)
regime_probabilities = calculate_regime_probability(regime_strength)

# Save regime probabilities and strengths to files
regime_prob_file = os.path.join(regimes_dir, 'regime_probabilities.nc')
regime_strength_file = os.path.join(regimes_dir, 'regime_strength.nc')

# Convert to xarray DataArray and save
time_coords = data_region['valid_time'].values
regime_prob_xr = xr.DataArray(regime_probabilities, dims=['time', 'regime'], 
                              coords={'time': time_coords, 'regime': np.arange(1, n_clusters + 1)})
regime_strength_xr = xr.DataArray(regime_strength, dims=['time', 'regime'], 
                                  coords={'time': time_coords, 'regime': np.arange(1, n_clusters + 1)})

regime_prob_xr.to_netcdf(regime_prob_file)
regime_strength_xr.to_netcdf(regime_strength_file)

# Save regime probabilities and strengths as pandas DataFrames and to pickle
regime_prob_df = pd.DataFrame(regime_probabilities, columns=[f'Regime_{i+1}' for i in range(n_clusters)], index=time_coords)
regime_strength_df = pd.DataFrame(regime_strength, columns=[f'Regime_{i+1}' for i in range(n_clusters)], index=time_coords)

regime_prob_df.to_pickle(os.path.join(regimes_dir, 'regime_probabilities.pkl'))
regime_strength_df.to_pickle(os.path.join(regimes_dir, 'regime_strengths.pkl'))

# Step 3: Compute the projection of geopotential anomalies onto EOFs (20 EOFs)
projections = project_onto_eofs(data_region, eofs)

# Step 4: Project onto regime centroids (7 regimes)
normalized_projections = project_onto_regimes(projections, centroids)

# Step 5: Save the normalized projections as the regime index (7 regimes)
regime_index_file = os.path.join(regimes_dir, 'regime_normalized_projection.nc')

# Convert to xarray DataArray and save
regime_index_xr = xr.DataArray(normalized_projections, dims=['time', 'regime'], 
                               coords={'time': time_coords, 'regime': np.arange(1, n_clusters + 1)})

regime_index_df = pd.DataFrame(regime_index_xr.values,
                               columns=regime_index_xr['regime'],
                               index=regime_index_xr['time'])
regime_index_df.to_pickle(os.path.join(regimes_dir, 'regime_index.pkl'))

regime_index_xr.to_netcdf(regime_index_file)

print(f'Regime normalized projections saved to {regime_index_file}')

# Assuming regime_strength is the array of strengths, normalize it
normalized_regime_strength = normalize_regime_strengths_by_mean_std(regime_strength)

# Save normalized strengths to file (optional)
normalized_strength_file = os.path.join(regimes_dir, 'normalized_regime_strength.nc')
normalized_regime_strength_xr = xr.DataArray(normalized_regime_strength, dims=['time', 'regime'], 
                                             coords={'time': time_coords, 'regime': np.arange(1, n_clusters + 1)})
normalized_regime_strength_xr.to_netcdf(normalized_strength_file)
