...
eof_dir = r'Z:\Data\Spot\Weather\Regimes'  # Directory where EOFs and centroids are stored
...
for new_file in sorted(os.listdir(new_data_dir)):
    if new_file.endswith('.nc') and int(new_file[-7:-3]) in range(2020, 2025):
        ...
        # Step 8: Normalize projections for each regime by its mean and std
        regime_means = regime_projections.mean(axis=0)  # Mean for each regime
        regime_stds = regime_projections.std(axis=0)    # Std deviation for each regime
        
        normalized_regime_projections = (regime_projections - regime_means) / regime_stds
