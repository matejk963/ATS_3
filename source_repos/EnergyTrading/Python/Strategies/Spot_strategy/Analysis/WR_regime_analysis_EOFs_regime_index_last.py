...
# # Step 3: Normalize the Projections for Comparison (Optional Normalization)
...
def normalize_projections(projections, centroids):
    # Transpose centroids to make their shape (20, 7) to match EOF space
    centroids_t = centroids  # Now centroids are (20, 7)

    # Project the projections back into EOF space
    projections_in_eof_space = np.dot(projections, centroids_t)  # Now (time, 20)
    
    # Calculate the Euclidean distance between projections and centroids
    distances = cdist(projections_in_eof_space, centroids, metric='euclidean')

    # Normalize by the mean and standard deviation of distances
