...
tol = 1e-6
trade_type_mm = {'open': 'MM', 'close': 'MID', 'trigg': 'CMB'}
...
class BacktestMM(BacktestClass):
    ...
    def simulate_strategy(self, strategy_class, method, instr, data_dict):
        ...
        if 'timestamp' not in price_dict.keys():
            price_dict['timestamp'] = price_dict['index']
        ...
        try:
            output_array = data_dict['output'].values
        except(AttributeError):
            output_array = data_dict['output']
        ...
        while self.is_active:
            ...
            if aux_dict['timestamp'].date() != self.prices_dict['timestamp'].date():
                self.prices_dict.update(aux_dict)
            aux_outs = output_array[self.current_t, :]
            price, vol = strategy_class.process(aux_dict, aux_outs, method,
