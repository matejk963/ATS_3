...
def adjust_std_based_on_distance(df, model_price, scaling_factor=1.0, alpha=2, beta=0.05):
    """
    ...
    for day, daily_df in df.groupby(df.index.date):
        ...
        # daily_df['lower band adjusted'] = daily_df['adjusted_price'] - daily_df['adjusted_std']
        # daily_df['upper band adjusted'] = daily_df['adjusted_price'] + daily_df['adjusted_std']
        
        # for i in [1,2]:
        #     daily_df['lower band adjusted_' + str(i+1)] = daily_df['lower band adjusted'] -\
        #        (i*daily_df['adjusted_std'])
        #     daily_df['upper band adjusted_' + str(i+1)] = daily_df['upper band adjusted'] +\
        #         (i*daily_df['adjusted_std'])
        
        # # # Append the adjusted data for this day
        # # # New cols names
        # # new_cols = ['mean-3sigma','mean-2sigma','mean-sigma',
        # #             'mean',
        # #             'mean+sigma','mean+2sigma','mean+3sigma',]
        # # adjusted_data.append(pd.DataFrame({
        # #     a: daily_df.iloc[:,b] for a, b in zip(new_cols,
        # #                                           range(daily_df.shape[1]))}))
        adjusted_data.append(pd.DataFrame({
...
import pickle
file_path = r'Z:\Data\Spot\MM\Inputs\input_data_simple_mm_20230901_20231130.pickle'
...
# Adjust trades
df_em = adjust_std_based_on_distance(ema_bands.copy(), model_price_df, scaling_factor=scaling_factor, beta=0.5)
