...
df_ba_a = df_ba.loc[df_ba.index.isin(df_em.index), :].copy()
df_ba_a = df_ba_a.loc[df_ba_a.index.date!=dt.datetime(2024,2,15)].copy()
df_tr_a = adjust_trds(df_tr.copy(), ema_bands.copy()).loc[:df_ba_a.index[-1]]
# df_em = model_input.copy()model_input = df_model_bounds.reset_index().copy()
...
vol_class = VolumeClass(1, {'max_clips': 6})
...
# Assuming 'timestamp' is the index in 'stats_df' or a column in 'stats_df'
