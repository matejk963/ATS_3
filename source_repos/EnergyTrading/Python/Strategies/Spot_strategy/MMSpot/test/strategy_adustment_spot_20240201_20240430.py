...
def adjust_std_based_on_distance(df, model_price, scaling_factor=1.0):
    ...
    for day, daily_df in df.groupby(df.index.date):
        ...
        adjusted_data.append(pd.DataFrame({
            'mean-3sigma': daily_df['lower band adjusted_3'],
            'mean-2sigma': daily_df['lower band adjusted_2'],
            'mean-sigma': daily_df['lower band adjusted'],
            'mean': daily_df.iloc[:,1],
            'mean+sigma': daily_df['upper band adjusted'],
            'mean+2sigma': daily_df['upper band adjusted_2'],
            'mean+3sigma': daily_df['upper band adjusted_3']
        }))
...
def get_last_trading_day_series(df):
    ...
    for week_ending in last_friday:
        if week_ending.date() in df.index.date:
            ...
            last_trading_day = pd.to_datetime(week_data.index.date[-1])
...
import pickle
file_path = r'Z:\Data\Spot\MM\Inputs\input_data_simple_mm_20240201_20240430.pickle'
...
data_dict[instr]['lt_date'] = get_last_trading_day_series(df_ba_a)
