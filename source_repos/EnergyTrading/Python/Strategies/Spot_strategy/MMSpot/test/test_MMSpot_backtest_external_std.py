...
def calc_ema_std_kernel_weekly(df_data, tau_fast, tau_slow,
    ...
    for day, group in df_data.groupby(pd.Grouper(freq='B')):
        ...
        if not prev_model_second_col.empty and hasattr(prev_model_second_col.index, 'date') and np.unique(prev_model_second_col.index.date).shape[0] >= std_window:
            # Calculate the rolling standard deviation over the last 5 days
            std0 = 4*prev_model_second_col.resample('D').last().dropna().rolling(std_window).mean().iloc[-1].values[0]
        else:
            # If we don't have enough data for 5 days yet, use a default value
            std0 = 4  # You can adjust this based on how you'd like to handle early days
        ...
        model_diff = model_list[0][:,0] - model_list[1][:,0]
        ...
        # Append the second column (m[1]) to prev_model_second_col for future rolling std calculation
        new_data = pd.DataFrame([m[1] for m in model_list[0]], index=group.index)
        prev_model_second_col = pd.concat([prev_model_second_col, new_data])
        ...
        if np.unique(prev_model_second_col.index.date).shape[0] >= std_window:
            ...
            for std_weight in std_weights:
                bands_group.append([price + (std_weight+ factor)*std if std_weight != 0
                                    else price
...
import pickle
file_path = r'Z:\Data\Spot\MM\Inputs\input_data_simple_mm_20240801_20240908.pickle'
...
for tau in [30]:
    for w_std in [1.5]:
        ...
        # emabands
        ema_bands = calc_ema_std_kernel_weekly(df_ba, 10, 50, w_std,N=30, tol=0)
        # Adjust trades
        ...
        # Sotore input data
        file_path = r'Z:\Data\Spot\MM\Inputs\input_data_simple_mm_20240501_20240801.pickle'
