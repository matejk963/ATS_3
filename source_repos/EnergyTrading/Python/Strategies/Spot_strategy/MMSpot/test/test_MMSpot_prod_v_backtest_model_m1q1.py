...
prod_data = pd.read_csv(r'C:\Users\krajcovic\Documents\Algo\Projects\Data\Algo\mm_de_m1q1_model_20241023.csv',
                        index_col=0)
...
start_time = time(14, 15, 0, 0)
end_time = time(17, 14, 39, 0)
...
exact_time = time(14, 27, 42, 0)
df_ba = df_ba.loc[df_ba.index.time>=exact_time].copy()
# emabands
...
w1_merged = pd.merge(de['m_1']['bid'], de['m_1']['ask'], left_index=True, right_index=True, how='outer', suffixes=('_bid_w1', '_ask_w1'))

# Outer merge bid and ask for w2 on index
w2_merged = pd.merge(de['q_1']['bid'], de['q_1']['ask'], left_index=True, right_index=True, how='outer', suffixes=('_bid_w2', '_ask_w2'))
...
df_legs = df_legs.loc[df_legs.index.time>=exact_time].copy()

df_spread = df_legs.copy()
df_spread['db_bid'] = df_spread['db_leg1_bid'] - df_spread['db_leg2_ask']
df_spread['db_ask'] = df_spread['db_leg1_ask'] - df_spread['db_leg2_bid']
df_spread['bid'] = (df_spread['leg1_bid'] - df_spread['leg2_ask'])
df_spread['ask'] = df_spread['leg1_ask'] - df_spread['leg2_bid']
df_spread['db_mid'] = ((df_spread['db_bid'] + df_spread['db_ask'])/2).ffill()
df_spread['mid'] = ((df_spread['bid'] + df_spread['ask'])/2).ffill()
...
model_data = pd.concat([prod_data[['model_p']],ema_bands[['ema_model']]],join='outer').sort_index()
