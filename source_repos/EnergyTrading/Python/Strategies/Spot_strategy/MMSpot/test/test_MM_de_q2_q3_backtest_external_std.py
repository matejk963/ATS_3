...
def calc_ema_std_weekly(df_data, tau, w_std, N=30, tol=0,
    ...
    for day, group in df_data.groupby(pd.Grouper(freq='B')):
        ...
        if np.unique(prev_model_second_col.index.date).shape[0] >= std_window:
            ...
            # bands.extend([[x - m, x, x + m, m] for x, m in zip(ema_list, mrg_list)])
            bands.extend([[x+(m*w) for w in std_weights]
                          for x, m in zip(ema_list, mrg_list)])
            

    return pd.DataFrame(bands, index=np.concatenate(bands_index)).ffill()

def calc_ema_std_weekly_multiple_bands(df_data, tau, w_std, N=30, tol=0, ):
    bands = []
    bands_index = []
    std_window = 5  # Rolling window of 5 days for std0
    prev_model_second_col = pd.DataFrame()  # To store second column of model_list
...
def calc_ema_std_kernel_weekly(df_data, tau_fast, tau_slow,
    ...
    def kernel_func(diff, std, kernel='tanh'):
        if kernel == 'log':
            ...
            adjustment_factor = 1 / (1 + np.exp(-diff))
        else:
    ...
    for day, group in df_data.groupby(pd.Grouper(freq='B')):
        ...
        if not prev_model_second_col.empty and hasattr(prev_model_second_col.index, 'date') and np.unique(prev_model_second_col.index.date).shape[0] >= std_window:
            # Calculate the rolling standard deviation over the last 5 days
            std0 = 2*prev_model_second_col.resample('D').last().dropna().rolling(std_window).mean().iloc[-1].values[0]
            
        else:
            # If we don't have enough data for 5 days yet, use a default value
            std0 = 2  # You can adjust this based on how you'd like to handle early days
        ...
        for tau in [tau_fast, tau_slow]:
            if tau == tau_fast:
                ...
                model = simple_model(MA(tau_slow, N), [tau_slow/2,N, mid_list[0]],
                                           'ema', burn=0, time_model=False, tol=tol)
        ...
        # Append the second column (m[1]) to prev_model_second_col for future rolling std calculation
        new_data = pd.DataFrame([m for m in model_list[0][:,1]], index=group.index, columns=['std'])        
        prev_model_second_col = pd.concat([prev_model_second_col, new_data])
        
        # Ensure that we start adding bands after 5 days of data
        if np.unique(prev_model_second_col.index.date).shape[0] >= std_window:
            
            macd = model_list[0][:,0] - np.nan_to_num(x=model_list[1],nan=model_list[1][1])
            model = simple_model(MA(tau_fast//2, n=3), [tau_fast//2/2,3, macd[0]],
                                       'ema', burn=0, time_model=False, tol=tol)
            signal_line = model.predict(macd)
            
            macd_hist = (macd - np.nan_to_num(x=signal_line,nan=signal_line[1]))/std0
            
            
            # Extract the first (ema) and second (mrg) elements from model_list
            ema_list = [x[0] for x in model_list[0]]
            factor_list = [kernel_func(diff, std, kernel) for std, diff in zip(model_list[0][:, 1],
                                                                                        macd_hist)]
...
for tau in [20]:
    for w_std in [1]:
        ...
        # emabands
        # ema_bands = calc_ema_std_kernel_weekly(df_ba, 25, 50, w_std,N=30, tol=0).bfill()
        ema_bands = calc_ema_std_weekly(df_ba, tau, w_std,N=30, tol=0.024).bfill()
        # Adjust trades
        ...
        # data_dict[instr]['lt_date'] = data_dict[instr]['lt_date'].apply(lambda x: x.replace(hour=16, minute=0, second=0) if pd.notna(x) else x)
        ...
        strategy_class = StrategyMM('MM', 'de', instr, is_overnight=True)
