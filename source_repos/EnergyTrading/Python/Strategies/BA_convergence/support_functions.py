...
def calculate_rolling_macd_fast(df, column, short_window=7, long_window=16):
    """
    Compute MACD and volatility while ignoring rows where 'trd%price' is not null in calculations.
    The computed values are forward-filled so all rows get a value.
    """
    # Create empty MACD and volatility series
    macd = pd.Series(index=df.index, dtype=float)
    volatility = pd.Series(index=df.index, dtype=float)

    # Filter valid rows where 'trd%price' is NULL
    valid_rows = df['trd_price'].isnull()

    # Compute short and long EMA only on valid rows
    short_ema = df.loc[valid_rows, column].ewm(span=short_window, adjust=False).mean()
    long_ema = df.loc[valid_rows, column].ewm(span=long_window, adjust=False).mean()

    # Compute MACD only for valid rows
    macd_valid = short_ema - long_ema

    # Compute volatility only for valid rows
    volatility_valid = df.loc[valid_rows, column].rolling(window=long_window).max() - df.loc[valid_rows, column].rolling(window=long_window).min()

    # Assign computed values back to all rows
    macd.loc[valid_rows] = macd_valid
    volatility.loc[valid_rows] = volatility_valid

    # Forward-fill to ensure all rows get values
