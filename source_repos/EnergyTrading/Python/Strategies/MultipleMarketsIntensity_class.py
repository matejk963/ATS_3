...
class MultiTradeIntensity:
    ...
    def prepare_data(self):
        ...
        for product in self.products:
            ...
            self.data[f'bidbestprice_{product}'] = self.data[f'bidbestprice_{product}']
            ...
            for product in self.products:
                ...
                for ba in ['bid', 'ask']:
                    ...
                    if ba in ['bid']:
                        ...
                        self.params_dict[product]['bid_params'] = np.mean(np.array(bid_params),axis=0)
                    elif ba in ['ask']:
                        ask_params.append(results.x)
                        self.params_dict[product]['ask_params'] = np.mean(np.array(ask_params),axis=0)
