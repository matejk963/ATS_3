...
def calculate_regression_model_price(df_lead, df_lag, tau, tau_ema, n_days):
    ...
    for current_day in dates_list:
        ...
        df_trds_1day = df_trds_1day.sort_values(by=['datetime', 'lag_volume', 'lag_price'], ascending=sort_order)
    ...
    for current_day in dates_list[n_days:]:
        ...
        df_trds_indexed.loc[df_trds_indexed['date'] == current_day, 'coef1'] = model.params[0]#0.00000000001#
        df_trds_indexed.loc[df_trds_indexed['date'] == current_day, 'coef2'] = model.params[1]#0.8661 #
...
def calc_vol_intensity_index(df, n, tag):
    ...
    for day, day_df in df.groupby("date"):
        ...
        lead_trades["cumcount"] = range(len(lead_trades)) # Cumulative count of trades
        ...
        lead_trades["time_since_last_n"]=lead_trades["time_diff"].rolling(window=n).sum()
