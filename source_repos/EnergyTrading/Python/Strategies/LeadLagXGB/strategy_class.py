...
class StrategyLL(StrategyClass):
    ...
    def process(self, price_dict, volm_class, trade_type):
        ...
        self.XGB_SCORE_SHORT=price_dict['XGB_SCORE_SHORT']
    ...
    def calculate_open_slot(self):
        ...
        if self.timestamp_.time() <= self.param_dict['t_end'] and self.lag_price_predicted:
            ...
            if not self.param_dict['combined_mode']:
                if self.MACD_12_26 > self.param_dict['MACD_long_threshold']  and (self.lag_price_predicted - self.ask_p) > self.param_dict['price_diff_long_threshold'] and self.lag_price_predicted>self.lag_price_tick and abs(self.VII_lag_5)>self.param_dict['minimum_intensity'] and abs(self.VII_lead_5)>self.param_dict['minimum_intensity']: # and self.lag_price_predicted>self.lag_price_tick
                    action = 1
                    price = self.ask_p
                elif self.MACD_12_26 <= self.param_dict['MACD_short_threshold'] and (self.lag_price_predicted - self.bid_p) < self.param_dict['price_diff_short_threshold'] and self.lag_price_predicted<self.lag_price_tick and abs(self.VII_lag_5)>self.param_dict['minimum_intensity'] and abs(self.VII_lead_5)>self.param_dict['minimum_intensity']: # and self.lag_price_predicted<self.lag_price_tick and self.MACD_30_60<-0.05
                    action = -1
                    price = self.bid_p
