...
class HawkesIntensity(ModelClass):
    ...
    def prepare_data(self):
        ...
        for product in self.products:
            ...
            # Bid true value
            self.data[f'bid_trade_{product}'] = np.where(self.data[f'action_{product}']==-1.,
                                              np.where(pd.notnull(self.data[f'vwp_{product}']),1,0),0)
            # Ask true value
            self.data[f'ask_trade_{product}'] = np.where(self.data[f'action_{product}']==1.,
                                              np.where(pd.notnull(self.data[f'vwp_{product}']),1,0),0)
    ...
    def create_dict(self, df):
        ...
        for product in self.products:
            aux_dict[product] = {}
            aux_dict[product]['trades'] = df[[f'vwp_{product}']].copy()
            # for product in self.products:
            for ba in ['bid', 'ask']:
    ...
    def estimate_params(self, train_lookback=30, no_of_it=1):
        ...
        keys_for_avg = list(data_dict)[:-1]
        keys_for_est = [list(data_dict)[-1]]
