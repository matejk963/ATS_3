# -*- coding: utf-8 -*-
"""
Created on Thu Aug  8 09:00:01 2024
...
bT = datetime(2024, 7, 28)
eT = datetime(2024, 9, 1) - timedelta(hours=1)
...
positions_['de'] = {'M': {'base': {datetime(2024,8,1): [[-1, 92.35],
                                                        [-1, 90.25],
                                                         [1, 89.4],
                                                         [-1, 89.5],
                                                         [-1, 81.77],
                                                         ...
                                                         [1,104.75],
                                                         [-1, 101.6]]}},
                    'W': {'base': {datetime(2024,8,19): [[-1*7/31, 79.9]],
                                   datetime(2024,8,26): [[-2*7/31, 98.1]]}}}
positions_['cz'] = {'M': {'base': {datetime(2024,8,1): [[-1, 65.5],
                                                        [1, 105.4]]}}}

positions_['fr'] = {'M': {'base': {datetime(2024,8,1): [[1, 87.75],
                                                        [1, 84.95],
                                                         [1, 83],
                                                         [1, 75.97],
                                                         [1, 56.66],
                                                         ...
                                                         [-1, 59.1],
                                                       [-1, 113.55]]}}}
positions_['hu'] = {'M': {'base': {datetime(2024,8,1): [[-1, 99.3],
                                                        [1, 91.25],
                                                       [-1, 115.45]]}},
                    'W': {'base': {datetime(2024,8,19): [[1*7/31, 100.0]],
                                   datetime(2024,8,26): [[2*7/31, 133.1]]}}}
positions_['at'] = {'M': {'base': {datetime(2024,8,1): [[-1, 115],
                                                       [-1, 110.55]]}}}
...
capa_strat.curtail_activity(sD, eD)
...
cur_df = pd.DataFrame({k: out_dict[k].iloc[-1, :] * v * 24 * 31 for k, v in vol_dict.items()}).T
print(cur_df)
print('\n\n')
print(hdg * 24 * 31)
print('\n\n')

pnl_df_c = pd.DataFrame({k: out_dict[k].iloc[:, 0] * v * 24 * 31 for k, v in vol_dict.items()}).T.sum()
...
mtm_capa = pd.DataFrame({k: out_dict[k].iloc[:, 1] * v * 24 * 31 for k, v in vol_dict.items()}).sum(axis=1)
mtm_hdg = hdg.iloc[:, 1] * 24 * 31
...
print(pd.DataFrame([pd.DataFrame({k: out_dict[k].loc[d, :] * v * 24 * 31
