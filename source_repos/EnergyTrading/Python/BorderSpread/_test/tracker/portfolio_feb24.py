...
bT = datetime(2024, 1, 28)
eT = datetime(2024, 3, 1) - timedelta(hours=1)
...
price_dict_y = {'sk_hu': 3, 'cz_de': 5, 'at_hu': 1, 'at_de': 5,
                'de_fr': 1}
...
positions_['de'] = {'M': {'base': {datetime(2024,2,1): [[-1, 96.6],
                                                        [-1, 97.1],
                                                        [1, 89.92],
                                                         [1, 81.5],
                                                         [-1, 82.15],
                                                         ...
                                                         [1,104.75],
                                                         [-1, 101.6]]}}}
positions_['cz'] = {'M': {'base': {datetime(2024,2,1): [[-1, 85.25],
                                                      [-1, 93.75],
                                                      [1, 105.4]]}}}

positions_['fr'] = {'M': {'base': {datetime(2024,2,1): [[1, 96.5],
                                                        [1, 80],
                                                        [1, 73.48],
                                                        [1, 66.12],
                                                       [-1, 113.55]]}}}
positions_['hu'] = {'M': {'base': {datetime(2024,2,1): [[1, 103.6],
                                                       [-1, 96.92],
                                                       [-1, 83.5],
                                                       [1, 81.94],
                                                       [-1, 98.14],
                                                       [-1, 115.45]]}}}
positions_['at'] = {'M': {'base': {datetime(2024,2,1): [[-1, 72],
                                                       [-1, 89.51],
                                                       [-1, 115],
                                                       [-1, 110.55]]}}}
...
cur_df = pd.DataFrame({k: out_dict[k].iloc[-1, :] * v * 24 * 29 for k, v in vol_dict.items()}).T
print(cur_df)
print('\n\n')
print(hdg * 24 * 29)
print('\n\n')

pnl_df_c = pd.DataFrame({k: out_dict[k].iloc[:, 0] * v * 24 * 29 for k, v in vol_dict.items()}).T.sum()
pnl_df_c.name = 'capPnl'
pnl_df_h = hdg.iloc[:, 0] * 24 * 29
...
mtm_capa = pd.DataFrame({k: out_dict[k].iloc[:, 1] * v * 24 * 29 for k, v in vol_dict.items()}).sum(axis=1)
mtm_hdg = hdg.iloc[:, 1] * 24 * 29
...
print(pd.DataFrame([pd.DataFrame({k: out_dict[k].loc[d, :] * v * 24 * 29
