# -*- coding: utf-8 -*-
"""
Created on Mon Mar  3 13:48:27 2025
...
bT = datetime(2025, 2, 28)
eT = datetime(2025, 4, 1) - timedelta(hours=1)
...
vol_dict_y = {'sk_hu': 3, 'cz_de': 5, 'at_hu': 1, 'at_de': 5,
              'de_fr': 1, 'gr_bg': 5}

price_dict_y = {'sk_hu': 6.5, 'cz_de': 2.6, 'at_hu': 14.56, 'at_de': 3.49,
                'de_fr': 4.35, 'gr_bg': 4.42}

vol_dict_m = {'cz_at': 3, 'at_cz': 3, 'sk_hu': 3,
              'de_at': 2, 'at_de': 1}

price_dict_m = {'cz_at': 6.79, 'at_cz': 3.5, 'sk_hu': 5.11,
                'de_at': 13.67, 'at_de': 1.05}

vol_dict = {key: vol_dict_y.get(key, 0) + vol_dict_m.get(key, 0) 
            for key in set(vol_dict_y) | set(vol_dict_m)}

# Get all unique keys
all_keys = set(vol_dict_y) | set(vol_dict_m)
# Compute volume-weighted average price
price_dict = {}
for key in all_keys:
    vol_y = vol_dict_y.get(key, 0)
    vol_m = vol_dict_m.get(key, 0)
    price_y = price_dict_y.get(key, 0)
    price_m = price_dict_m.get(key, 0)
    
    total_volume = vol_y + vol_m
    if total_volume > 0:
        price_dict[key] = (vol_y * price_y + vol_m * price_m) / total_volume
    else:
        price_dict[key] = 0  # This should never happen with the given data
...
positions_['de'] = {'M': {'base': {datetime(2025,3,1): [[-2, 96.84]]}}}
positions_['cz'] = {'M': {'base': {datetime(2025,3,1): [[2, 100.55]]}}}

# positions_['fr'] = {'M': {'base': {datetime(2025,2,1): [[1, 96.5],
#                                                         [1, 80],
#                                                         [1, 73.48],
#                                                         [1, 66.12],
#                                                         [-1, 113.55]]}}}
# positions_['hu'] = {'M': {'base': {datetime(2025,2,1): [[1, 103.6],
#                                                         [-1, 96.92],
#                                                         [-1, 83.5],
#                                                         [1, 81.94],
#                                                         [-1, 98.14],
#                                                         [-1, 115.45]]}}}
# positions_['at'] = {'M': {'base': {datetime(2025,2,1): [[-1, 72],
#                                                         [-1, 89.51],
#                                                         [-1, 115],
#                                                         [-1, 110.55]]}}}
...
cur_df = pd.DataFrame({k: out_dict[k].iloc[-1, :] * v * 24 * n_days for k, v in vol_dict.items()}).T
print(cur_df)
print('\n\n')
print(hdg * 24 * n_days)
print('\n\n')

pnl_df_c = pd.DataFrame({k: out_dict[k].iloc[:, 0] * v * 24 * n_days for k, v in vol_dict.items()}).T.sum()
pnl_df_c.name = 'capPnl'
pnl_df_h = hdg.iloc[:, 0] * 24 * n_days
...
mtm_capa = pd.DataFrame({k: out_dict[k].iloc[:, 1] * v * 24 * n_days for k, v in vol_dict.items()}).sum(axis=1)
mtm_hdg = hdg.iloc[:, 1] * 24 * n_days
...
print(pd.DataFrame([pd.DataFrame({k: out_dict[k].loc[d, :] * v * 24 * n_days
                                  for k, v in vol_dict.items()}).T.sum().sum() + (hdg.loc[d, :] * 24 * n_days).sum().sum()
