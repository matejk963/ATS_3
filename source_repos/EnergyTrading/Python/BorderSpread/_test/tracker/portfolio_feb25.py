...
bT = datetime(2025, 1, 28)
eT = datetime(2025, 3, 1) - timedelta(hours=1)
...
positions_['de'] = {'M': {'base': {datetime(2025,2,1): [[-2, 96.84]]}}}
positions_['cz'] = {'M': {'base': {datetime(2025,2,1): [[2, 100.55]]}}}
...
cur_df = pd.DataFrame({k: out_dict[k].iloc[-1, :] * v * 24 * 28 for k, v in vol_dict.items()}).T
print(cur_df)
print('\n\n')
print(hdg * 24 * 28)
print('\n\n')

pnl_df_c = pd.DataFrame({k: out_dict[k].iloc[:, 0] * v * 24 * 28 for k, v in vol_dict.items()}).T.sum()
pnl_df_c.name = 'capPnl'
pnl_df_h = hdg.iloc[:, 0] * 24 * 28
...
mtm_capa = pd.DataFrame({k: out_dict[k].iloc[:, 1] * v * 24 * 28 for k, v in vol_dict.items()}).sum(axis=1)
mtm_hdg = hdg.iloc[:, 1] * 24 * 28
...
print(pd.DataFrame([pd.DataFrame({k: out_dict[k].loc[d, :] * v * 24 * 28
                                  for k, v in vol_dict.items()}).T.sum().sum() + (hdg.loc[d, :] * 24 * 28).sum().sum()
